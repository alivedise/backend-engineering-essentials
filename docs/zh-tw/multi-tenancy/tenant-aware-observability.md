...
- [BEE-18001](multi-tenancy-models.md) -- 多租戶架構模型：孤島/共享池/橋接——隔離模型決定遙測後端是否可以共享或必須按租戶獨立
- [BEE-18002](tenant-isolation-strategies.md) -- 租戶隔離策略：數據層隔離（RLS、命名空間）和控制層隔離（本文重點：遙測）
- [BEE-18003](tenant-aware-rate-limiting-and-quotas.md) -- 租戶感知速率限制：速率限制決策應作為按租戶標記的配額消耗指標反饋到可觀測性中
- [BEE-14001](../observability/three-pillars-logs-metrics-traces.md) -- 三大支柱：日誌、指標、追蹤：基礎可觀測性概念；本文用租戶感知維度加以擴展
- [BEE-14005](../observability/slos-and-error-budgets.md) -- SLO 和錯誤預算：每租戶 SLO 需要每租戶錯誤率指標作為其輸入信號
