...
- 垂直擴展有上限；水平分片可行但增加複雜度（參見 [BEE-6004](partitioning-and-sharding.md)）。
...
SQL 資料庫確實可以擴展 — PostgreSQL 透過適當的索引、讀取副本和連線池可處理每秒數百萬次查詢。水平分片複雜但可行（參見 [BEE-6004](partitioning-and-sharding.md)）。在沒有對實際瓶頸進行基準測試的情況下選擇 NoSQL 來「避免擴展問題」是過早最佳化。
...
- [BEE-6002](indexing-deep-dive.md) — 索引深度探討：SQL 與 NoSQL 系統中索引的運作方式
- [BEE-6003](replication-strategies.md) — 複寫策略：主從複寫、多主複寫、最終一致性
- [BEE-6004](partitioning-and-sharding.md) — 分區與分片：大型資料集的水平擴展策略
- [BEE-7001](../data-modeling/entity-relationship-modeling.md) — 資料建模：將領域概念轉換為資料庫結構
