...
- **防止過載** — 作為 API 層的負載卸載閥（參見 [BEE-264](264.md)），防止流量突增向資料庫及下游服務蔓延
...
從客戶端角度（以及服務呼叫其他服務時），收到 429 並非傳統意義上的錯誤，而是退讓的信號。完整的重試退讓指引參見 [BEE-261](261.md)。關鍵要點：
...
- [BEE-15](15.md)（API 金鑰）— API 金鑰是每客戶端限流的主要識別符；金鑰輪換與範圍限定影響限制執行
- [BEE-75](75.md)（錯誤處理）— 429 是應用程式層面的錯誤，必須明確處理，不得視為意外例外
- [BEE-261](261.md)（重試策略與指數退讓）— 收到 429 後的重試行為；始終包含抖動並遵守 `Retry-After`
- [BEE-264](264.md)（優雅降級與負載卸載）— 限流作為負載卸載的一層；[BEE-264](264.md) 涵蓋應用程式層面的卸載策略
