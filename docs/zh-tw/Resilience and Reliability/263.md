...
這就是**資源耗盡引發的級聯故障**。這個問題隱蔽且常見。斷路器（BEE-260）在依賴*故障*時才會啟動；它無法保護你免於依賴*緩慢*的衝擊。逾時設定（BEE-261）限制單次呼叫的等待時間，但若 500 個執行緒同時各等待最多 5 秒，執行緒池仍會在任何逾時觸發之前就被耗盡。
...
1. **逾時**（BEE-261）——限制單次呼叫的阻塞時間。
2. **艙壁**——限制每個依賴的並發進行中呼叫數量。
3. **斷路器**（BEE-260）——停止呼叫故障或飽和的依賴。
...
- **BEE-241**（Worker Pools）——執行緒池大小設定與管理的基礎概念
- **BEE-260**（斷路器模式）——停止呼叫故障依賴；與艙壁搭配使用以提供完整保護
- **BEE-261**（逾時與截止時間）——限制呼叫持續時間，避免分區槽被無限期佔用
- **BEE-261**（優雅降級）——定義艙壁分區已滿時的後備行為
