...
2. 操作支援冪等鍵，讓伺服器可以去重（參見 BEP-71 與 BEP-161）
...
3. **使用斷路器**（BEP-260）— 當下游服務確認故障時，完全停止重試
4. **搭配逾時限制**（BEP-261）— 有界的逾時設定可防止重試堆疊延遲
...
在沒有冪等鍵的情況下，對支付或建立訂單的端點加入重試邏輯，會導致重複交易。在重試前請務必確認冪等性（BEP-71、BEP-161）。
...
- **BEP-71** — 冪等鍵，讓重試安全可行
- **BEP-161** — 透過去重實現精確一次傳遞
- **BEP-260** — 斷路器，在下游服務故障時停止重試
- **BEP-261** — 逾時設定，限制重試的延遲上限
