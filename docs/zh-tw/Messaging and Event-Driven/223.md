...
- **搭配 CQRS（[BEE-102](102.md)）使用** — 事件溯源是 CQRS 讀寫分離中寫入端的自然補充。
...
- [BEE-102](102.md) — CQRS：讀寫模型分離；事件溯源是寫入端的自然配對
- [BEE-144](144.md) — 時態資料：查詢某個時間點的狀態
- [BEE-164](164.md) — 冪等性：投影器必須具備冪等性，才能安全地支援重播
- [BEE-220](220.md) — 事件流：在生產者與投影器之間傳遞事件的基礎設施層
