...
這是大多數系統的標準選擇。消費者必須設計為能安全處理重複訊息——透過設計上的冪等性，或依據穩定的訊息 ID 明確去重（見 BEE-226）。
...
這是區分暫時性失敗（服務不可用）與永久性失敗（訊息格式錯誤）的正確模式。DLQ 設計見 BEE-224。
...
這就是為什麼 BEE-164（冪等性）和 BEE-226（冪等處理）需要與交付保證並存——兩個層次都需要。
...
選擇 at-least-once delivery 但未設計重複處理，是最常見的故障模式。若消費者執行 `INSERT INTO orders (id, ...) VALUES (...)` 而沒有去重檢查，重新傳遞的訊息會插入重複的行。始終將 at-least-once delivery 與冪等性配對使用（見 BEE-164、BEE-226）。
...
- **BEE-164** — 冪等性：設計可以安全重複執行的操作
- **BEE-220** — 訊息佇列 vs 事件串流：選擇合適的模型
- **BEE-224** — 死信佇列與有毒訊息
- **BEE-226** — 冪等訊息處理：消費者中的去重模式
