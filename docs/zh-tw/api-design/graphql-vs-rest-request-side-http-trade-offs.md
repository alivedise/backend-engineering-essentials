...
[BEE-4010](graphql-http-caching.md) 建立了基本框架：GraphQL 並非以 HTTP 為前提設計。它的標準傳輸方式是 `POST /graphql`，把操作放在請求主體，並且不論讀或寫哪個邏輯資源，都從同一個端點服務。這個設計換來 schema 驅動的彈性，卻交出 REST 從 HTTP 本身免費取得的三樣東西。
