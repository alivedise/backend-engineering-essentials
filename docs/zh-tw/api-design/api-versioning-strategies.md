...
Introspection 會暴露 `__Field.isDeprecated` 與 `__Field.deprecationReason`，讓 IDE（GraphiQL）、程式碼產生器、linter 不需要 release notes 就能把訊號傳達給消費方。欄位 deprecation 在 wire 上是非破壞性的：規格 §3.6.2「Field Deprecation」明載 deprecated 欄位仍可被合法選取，讓消費方按自己的節奏遷移。
...
```mermaid
flowchart TD
    Z[新 API — 選擇版本控制策略] --> Y{GraphQL 或 REST？}
    Y -->|GraphQL| YG[加性演進 + @deprecated]
    YG --> YGQ{消費方規模過大\n無法協調 deprecation？}
    YGQ -->|否| YGA[持續 schema 演進]
    YGQ -->|是| YGB[日曆式 schema 切版\nShopify 2026-04 模型]
