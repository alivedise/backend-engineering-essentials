...
|---|---|---|
| **錯誤語意** | HTTP 狀態碼 + RFC 9457 Problem Details（BEE-75） | GraphQL-over-HTTP 狀態碼對應 + `errors[].extensions.code` + partial-success 合約 |
...
**GraphQL 缺口。** 預設行為不論失敗模式為何都是 `200 OK`，錯誤放進 `data.errors[]`。這就是 BEE-75 點名為最具破壞性的錯誤處理錯誤的同一個「200 加 success flag 在主體裡」反模式，因為它同時破壞每一個感知 HTTP 的工具。
