...
契約測試在測試金字塔中位於單元測試與整合測試之間（見 [BEE-15001](testing-pyramid.md)）。它比整合測試更快、更便宜，因為它不需要共享環境。它涵蓋了單元測試無法處理的缺口：兩個獨立部署服務之間的通訊邊界。
...
付款服務 CI 失敗。這個變更在部署前被阻擋。付款服務開發者現在知道訂單服務依賴 `amount`，必須在進行此變更前與該團隊協調。他們有兩個選項：保留該欄位，或與訂單服務協商一個新的契約版本（API 版本控制策略請見 [BEE-4002](../api-design/api-versioning-strategies.md)）。
...
基於 schema 的契約測試與 API 優先開發（見 [BEE-4002](../api-design/api-versioning-strategies.md)）整合良好：先設計 schema，消費者和提供者團隊都依照它實作，schema 成為共同的信任來源。
...
使用契約測試來及早、持續地發現結構不匹配。使用整合測試來驗證元件連接後的正確行為（見 [BEE-15002](integration-testing-for-backend-services.md)）。使用 E2E 測試驗證完整的使用者旅程。
...
- [BEE-4002](../api-design/api-versioning-strategies.md)（API 版本控制）——如何在不破壞消費者的前提下演進提供者 API；契約測試讓破壞性變更變得可偵測
- [BEE-7003](../data-modeling/schema-evolution-and-backward-compatibility.md)（Schema 演進）——以保持向下相容性的方式演進資料 schema 的策略
- [BEE-15001](testing-pyramid.md)（測試金字塔）——契約測試在整合測試與單元測試之間的位置
- [BEE-15002](integration-testing-for-backend-services.md)（後端服務整合測試）——在契約測試驗證結構的地方測試行為的互補層
