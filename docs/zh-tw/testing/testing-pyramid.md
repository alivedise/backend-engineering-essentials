...
單元測試針對單一函式、類別或模組進行隔離測試。外部依賴（資料庫、網路呼叫、檔案系統）以測試替身（Test Double）取代（參見 [BEE-15005](test-doubles-mocks-stubs-fakes.md)）。
...
- [BEE-15002](integration-testing-for-backend-services.md)（後端服務的整合測試）— 使用真實資料庫和測試容器撰寫整合測試的詳細指引
- [BEE-15003](contract-testing.md)（合約測試）— 無需執行完整 E2E 套件即可驗證跨服務合約
- [BEE-15004](load-testing-and-benchmarking.md)（負載測試與基準測試）— 功能測試金字塔之外的效能驗證
- [BEE-15005](test-doubles-mocks-stubs-fakes.md)（測試替身：Mock、Stub、Fake）— 如何隔離金字塔底層的單元，同時不損害測試價值
