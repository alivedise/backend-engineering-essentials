...
- [BEE-5004 — 六邊形架構](../architecture-patterns/hexagonal-architecture.md)：Port 是天然的 Mock 邊界。Mock Port 介面，而非其背後的物件。
- [BEE-15001 — 測試金字塔](testing-pyramid.md)：使用替身的單元測試構成底層；金字塔決定各類型應寫多少。
- [BEE-15002 — 整合測試](integration-testing-for-backend-services.md)：何時不應 Mock — 需要真實基礎設施才能提供有意義測試覆蓋率的情況。
