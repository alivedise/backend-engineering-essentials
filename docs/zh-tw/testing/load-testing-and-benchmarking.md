...
容量測試找出理論上的上限：服務在維持 SLO 範圍內所能承受的最大吞吐量。這對於基礎設施規劃（3 倍目前流量需要多少實例？）和設定優化工作目標很有用（請參閱 [BEE-13004](../performance-scalability/profiling-and-bottleneck-identification.md) 的效能分析）。
...
P99 是 SLO 驗證中最重要的單一延遲數字（請參閱 [BEE-14005](../observability/slos-and-error-budgets.md)）。將 SLO 設在 P99 而非平均值，並在 P99 超過閾值時觸發警報。
...
基準測試屬於單元測試和效能分析工作階段（請參閱 [BEE-13004](../performance-scalability/profiling-and-bottleneck-identification.md)）。負載測試屬於 CI/CD 流程，作為系統準備就緒的門檻。
...
負載測試是驗證服務的 SLO（請參閱 [BEE-14005](../observability/slos-and-error-budgets.md)）是否可實現而非僅是願景的主要機制。
...
相關用途：**容量規劃**。如果目前服務在 4 核心實例上以 1000 RPS 符合 SLO，負載測試可以確定 2000 RPS 是否需要 8 核心、2 個實例或資料庫索引變更。負載測試為估算（請參閱 [BEE-13001](../performance-scalability/back-of-envelope-estimation.md)）提供定量基礎。
...
- [BEE-13001](../performance-scalability/back-of-envelope-estimation.md)（估算）——負載測試以量測數據驗證容量估算；使用測試結果來校準估算
- [BEE-13004](../performance-scalability/profiling-and-bottleneck-identification.md)（效能分析）——當負載測試揭露效能問題時，效能分析找出哪個程式碼路徑是瓶頸
- [BEE-14005](../observability/slos-and-error-budgets.md)（SLO）——負載測試是驗證 SLO 可實現並持續執行的主要機制
