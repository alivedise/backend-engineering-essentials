...
- [BEE-380](380.md) -- 全文搜尋基礎：倒排索引、分析管線和 BM25 評分，是實作多面向搜尋所用搜尋引擎的基礎。
- [BEE-381](381.md) -- 搜尋相關性調校：如何衡量並改善面向所操作的命中集合中的結果排名。
- [BEE-73](../API Design and Communication Protocols/73.md) -- 分頁模式：多面向搜尋回應需要分頁；偏移量、游標和 keyset 分頁均適用。
- [BEE-202](../Caching/202.md) -- 快取驅逐策略：熱門的面向查詢可受益於聚合結果的應用層快取。
