...
Goroutine 很便宜，但不是免費的。在沒有背壓機制的情況下為每個進入的請求生成一個 goroutine，在流量尖峰時將耗盡記憶體。務必以號誌、工作者池或速率限制器限制並行度。工作者池模式請參見 [BEE-11005](producer-consumer-and-worker-pool-patterns.md)。
...
- [BEE-11002](race-conditions-and-data-races.md) — 競爭條件：執行緒在未同步的情況下共享狀態時會發生什麼
- [BEE-11003](locks-mutexes-and-semaphores.md) — 鎖與互斥：互斥鎖模式、死鎖與鎖的排序
- [BEE-11004](async-i-o-and-event-loops.md) — Async I/O：事件迴圈、非阻塞系統呼叫與 Reactor 模式
- [BEE-11005](producer-consumer-and-worker-pool-patterns.md) — 工作者池：限制並行度、背壓與優雅關閉
