...
│   │   └── 工作池大小 = 核心數 × (1 + 等待/運算)
│   │       或使用非同步 I/O（見 [BEE-11004](async-i-o-and-event-loops.md)）
│   │
│   └── 同一服務中有混合工作負載？
│       └── 每種工作負載類型使用獨立工作池
│
└── 沒有離散任務（串流、持續運算）？
    └── 考慮非同步 I/O（[BEE-11004](async-i-o-and-event-loops.md)）或響應式串流（[BEE-13006](../performance-scalability/asynchronous-processing-and-work-queues.md)）
```

## 相關 BEPs

- [BEE-10006](../messaging/backpressure-and-flow-control.md) — 背壓：向上游傳播負載訊號的系統級策略
- [BEE-11001](threads-vs-processes-vs-coroutines.md) — 執行緒 vs 行程 vs 協程：工作池建立的基礎原語
- [BEE-11004](async-i-o-and-event-loops.md) — 非同步 I/O 與事件迴圈：I/O 密集型工作的執行緒池替代方案
- [BEE-13006](../performance-scalability/asynchronous-processing-and-work-queues.md) — 非同步處理與作業佇列：跨服務任務分發的持久分散式佇列（Kafka、SQS）
