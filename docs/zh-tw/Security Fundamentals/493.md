...
- [BEE-13](../Authentication and Authorization/13.md) -- Session 管理：session cookie 是 CSRF 的攻擊面；`httpOnly` 和 `Secure` 旗標是 cookie 安全的前提條件
- [BEE-33](33.md) -- CORS 與同源策略：CORS 預檢是使自訂標頭防禦有效的機制；同源策略是 CSRF 需要 cookie 向量的原因
- [BEE-489](489.md) -- HTTP 安全標頭：`SameSite` 存在於 `Set-Cookie` 中；CSRF / CSP 的交互（XSS 破壞所有 CSRF 防禦）
- [BEE-488](488.md) -- OWASP API 安全 Top 10：當 API 認證依賴 cookie 而非明確令牌時，CSRF 會浮現
