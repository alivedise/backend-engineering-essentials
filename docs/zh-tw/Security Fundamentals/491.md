...
- [BEE-34](34.md) -- 工程師的密碼學基礎：憑證金鑰對和簽名鏈背後的非對稱密碼學（RSA、ECDSA）
- [BEE-53](../Networking/53.md) -- TLS/SSL 握手：使用憑證的協定機制——連線時憑證如何被出示和驗證
- [BEE-467](../Distributed Systems/467.md) -- 服務間身份驗證：作為認證策略的 mTLS；本文涵蓋使 mTLS 運作的 PKI 層
- [BEE-490](490.md) -- 密碼學金鑰管理與金鑰輪換：憑證背後的私鑰受所有金鑰管理要求約束——DEK/KEK、HSM 儲存、密碼期
