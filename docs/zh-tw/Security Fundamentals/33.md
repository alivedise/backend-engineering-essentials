...
- **BEP-30** — OWASP Top 10 for Backend：CORS 錯誤設定屬於存取控制缺陷（A01）與安全性錯誤設定（A05）。
- **BEP-50** — Networking Fundamentals：TCP/IP 與 HTTP 請求路由背景。
- **BEP-51** — HTTP Semantics：支撐 CORS 行為的請求方法、狀態碼與 header 慣例。
