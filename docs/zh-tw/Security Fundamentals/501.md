...
- [BEE-32](32.md) -- Secrets 管理：PII 字段的加密金鑰是 Secrets，必須相應地管理——絕不與它們所保護的資料儲存在一起
- [BEE-460](460.md) -- 軟刪除與資料保留：軟刪除本身無法滿足被遺忘權——軟刪除的記錄仍然包含 PII；刪除工作流程必須移除或匿名化 PII 字段
- [BEE-462](462.md) -- 審計日誌架構：審計日誌必須記錄抹除請求而不保留被抹除的 PII；將審計架構設計為引用內部 ID，而非個人資料
- [BEE-476](476.md) -- 資料庫行層級安全：RLS 可以強制應用程式使用者只能讀取自己的 PII 行，減少 BOLA 漏洞對 PII 資料的影響範圍
- [BEE-143](143.md) -- 編碼與序列化格式：預設包含所有物件字段的序列化函式庫可能在 API 響應中意外暴露 PII——使用明確的字段允許清單
