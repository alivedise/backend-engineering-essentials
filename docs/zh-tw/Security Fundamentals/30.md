...
後端相關性：存取控制必須在每個請求的伺服器端強制執行。參見 [BEE-10](10.md)。
...
後端相關性：靜態資料（data-at-rest）與傳輸中資料（data-in-transit）的每個決策都涉及加密。參見 [BEE-34](34.md)。
...
後端相關性：注入攻擊鎖定資料層，以及任何從輸入建構命令字串的系統呼叫。參見 [BEE-31](31.md)。
...
後端相關性：設定必須按環境稽核；正式環境（production）與開發環境（development）的設定必須有所不同。CORS 細節參見 [BEE-33](33.md)。
...
後端相關性：相依套件清單（dependency inventory）、自動漏洞掃描與及時修補，是不可忽視的必要工作。參見 [BEE-35](35.md)。
