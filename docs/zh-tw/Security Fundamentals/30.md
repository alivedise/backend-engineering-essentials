...
後端相關性：存取控制必須在每個請求的伺服器端強制執行。參見 BEE-10。
...
後端相關性：靜態資料（data-at-rest）與傳輸中資料（data-in-transit）的每個決策都涉及加密。參見 BEE-34。
...
後端相關性：注入攻擊鎖定資料層，以及任何從輸入建構命令字串的系統呼叫。參見 BEE-31。
...
後端相關性：設定必須按環境稽核；正式環境（production）與開發環境（development）的設定必須有所不同。CORS 細節參見 BEE-33。
...
後端相關性：相依套件清單（dependency inventory）、自動漏洞掃描與及時修補，是不可忽視的必要工作。參見 BEE-35。
...
- [BEE-10: Authentication vs Authorization](/zh-tw/Authentication%20and%20Authorization/10) — 基礎存取控制流水線（A01、A07）
- [BEE-31: Input Validation and Sanitization](/zh-tw/Security%20Fundamentals/31) — 防禦注入攻擊（A03）
- [BEE-32: Secrets Management](/zh-tw/Security%20Fundamentals/32) — 保護憑證與金鑰（A02）
- [BEE-33: CORS and Same-Origin Policy](/zh-tw/Security%20Fundamentals/33) — 瀏覽器端存取控制設定（A05）
- [BEE-34: Cryptographic Basics for Engineers](/zh-tw/Security%20Fundamentals/34) — 加密與雜湊基礎（A02）
- [BEE-35: Dependency Security and Supply Chain](/zh-tw/Security%20Fundamentals/35) — 管理第三方風險（A06）
