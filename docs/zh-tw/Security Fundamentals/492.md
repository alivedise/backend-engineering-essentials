...
- [BEE-30](30.md) -- 後端的 OWASP Top 10：A03:2021 注入是傘形類別；本文涵蓋 SQL 特定的實作機制
- [BEE-31](31.md) -- 輸入驗證與清理：處理不受信任輸入的更廣泛原則；參數化是資料庫邊界的決定性防禦
- [BEE-488](488.md) -- OWASP API 安全 Top 10：SQL 注入在 API 情境中作為風險出現（API10：處理第三方資料時的不安全消費）
