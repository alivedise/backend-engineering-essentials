...
- [BEE-30](30.md) -- OWASP Top 10 後端：STRIDE 威脅直接對應 OWASP Top 10 類別——注入（T）、存取控制失效（E/I）、安全配置錯誤（D/I）
- [BEE-497](497.md) -- SSRF：在 API 服務可存取內部網路端點的 DFD 中，被識別為權限提升和資訊洩露威脅
- [BEE-499](499.md) -- BOLA：在資料儲存存取點上，當未驗證物件層級所有權時，是一種資訊洩露威脅
- [BEE-501](501.md) -- 資料隱私與 PII 處理：針對 PII 的資訊洩露威脅，決定哪些資料儲存需要加密和日誌清理
- [BEE-462](462.md) -- 稽核日誌架構：透過確保每個敏感操作都被記錄在防竄改稽核日誌中來緩解否認威脅
