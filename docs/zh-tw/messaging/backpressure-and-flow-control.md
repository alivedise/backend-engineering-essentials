...
斷路器（見 [BEE-12001](../resilience/circuit-breaker-pattern.md)）在下游錯誤率或延遲超過閾值時開路，阻止對降級服務的請求。這是服務呼叫層面的背壓。它保護呼叫方不把資源浪費在緩慢的下游，但不減少下游的負載——而是重新導向。將斷路器與降級方案配對，而非僅與錯誤配對。
...
速率限制（見 [BEE-12007](../resilience/rate-limiting-and-throttling.md)）約束單一客戶端的請求速率。背壓根據下游容量約束資料流速率。它們解決不同的問題：
...
- [BEE-10001](message-queues-vs-event-streams.md) — 訊息基礎：背壓整合其中的代理模式
- [BEE-12001](../resilience/circuit-breaker-pattern.md) — 斷路器：補充背壓的服務層級保護
- [BEE-12007](../resilience/rate-limiting-and-throttling.md) — 速率限制：請求速率控制，一種靜態形式的背壓
- [BEE-13006](../performance-scalability/asynchronous-processing-and-work-queues.md) — 非同步處理：結構上需要背壓的模式
