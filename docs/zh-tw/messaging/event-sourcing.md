...
- **搭配 CQRS（[BEE-5003](../architecture-patterns/cqrs.md)）使用** — 事件溯源是 CQRS 讀寫分離中寫入端的自然補充。
...
- [BEE-5003](../architecture-patterns/cqrs.md) — CQRS：讀寫模型分離；事件溯源是寫入端的自然配對
- [BEE-7005](../data-modeling/designing-for-time-series-and-audit-data.md) — 時態資料：查詢某個時間點的狀態
- [BEE-8005](../transactions/idempotency-and-exactly-once-semantics.md) — 冪等性：投影器必須具備冪等性，才能安全地支援重播
- [BEE-10001](message-queues-vs-event-streams.md) — 事件流：在生產者與投影器之間傳遞事件的基礎設施層
