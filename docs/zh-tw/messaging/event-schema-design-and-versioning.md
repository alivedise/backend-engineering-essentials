...
- [BEE-7004](../data-modeling/encoding-and-serialization-formats.md) -- 編碼與序列化格式：涵蓋格式層面的 Avro、Protobuf 和 JSON；本文涵蓋這些格式之上的綱要演進策略
- [BEE-10001](message-queues-vs-event-streams.md) -- 訊息佇列 vs 事件串流：事件串流（Kafka）具有與訊息佇列不同的綱要需求，因為事件被保留和重播
- [BEE-10003](delivery-guarantees.md) -- 傳遞保證：精確一次語義與綱要版本控制互動——重試訊息的消費者必須處理原始生產時的綱要版本
- [BEE-19053](../distributed-systems/outbox-pattern-and-transactional-messaging.md) -- 發件箱模式與事務性訊息：發件箱事件是綱要演進壓力的主要來源；發件箱表綱要和事件綱要必須一起演進
