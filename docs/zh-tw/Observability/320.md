...
非結構化的日誌行無法按欄位搜尋或聚合。當事故發生時，你無法篩選 `level=error AND service=order-service AND trace_id=X`，只能在凌晨兩點 grep 純文字。請務必使用 JSON 或結構化格式記錄日誌。詳見 BEE-321。
...
- **BEE-321** — 結構化日誌：schema、日誌等級，以及什麼不該記錄
- **BEE-322** — 分散式追蹤：span、取樣策略與 context propagation
- **BEE-323** — 告警哲學：應該告警什麼，以及如何撰寫好的 runbook
- **BEE-324** — SLO 與錯誤預算：使用指標定義並追蹤可靠性目標
