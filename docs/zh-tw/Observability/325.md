...
健康檢查與熔斷器（參見 BEP-260）都共享依賴健康狀態的資訊，但服務的消費方不同。
...
當 Kubernetes 發送 `SIGTERM` 時，Pod 進入終止序列。此時就緒探針應立即開始失敗，讓負載平衡器在 Pod 關閉連線之前就停止向其路由新請求。完整的優雅降級序列請參見 BEP-261。
...
Kubernetes 之外的負載平衡器——ALB、NGINX、HAProxy——同樣需要健康檢查設定。負載平衡器健康檢查的細節請參見 BEP-51。
...
- **BEP-51** — 負載平衡器健康檢查：在負載平衡器層面設定健康檢查間隔和閾值。
- **BEP-260** — 熔斷器：開路的熔斷器應如何影響就緒狀態。
- **BEP-261** — 優雅降級：協調關閉期間 SIGTERM 處理與就緒探針狀態。
