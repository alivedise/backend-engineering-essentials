...
- [BEE-5004](hexagonal-architecture.md) -- 六角形架構：端口和適配器依賴 DI 在執行時為每個端口注入正確的適配器
- [BEE-5002](domain-driven-design-essentials.md) -- 領域驅動設計基礎：DI 透過為領域聚合注入正確的儲存實作來啟用儲存庫模式
- [BEE-15005](../testing/test-doubles-mocks-stubs-fakes.md) -- 測試替身：Mock、Stub、Fake：DI 是測試替身的前提——一個自己構建依賴的類別無法讓它們被假物件替換
- [BEE-5003](cqrs.md) -- CQRS：命令和查詢處理器通常在 DI 容器中注冊為 Scoped 服務，每個請求有獨立的處理器實例
