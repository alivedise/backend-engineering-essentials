...
**應用程式存在的目的是實作業務規則**。其他一切 -- HTTP、資料庫、訊息佇列、CLI -- 都是基礎設施。六角形架構讓兩者之間的邊界明確且可強制執行。
...
- **DDD**（參見 [BEE-5002](domain-driven-design-essentials.md)）-- 六角形架構是 DDD 的天然結構補充。領域聚合、實體和值物件存在於核心；Repository 是被驅動端 Port。
- **CQRS**（參見 [BEE-5002](domain-driven-design-essentials.md)）-- 命令與查詢處理器是領域核心的使用案例；讀模型 Adapter 和寫 Adapter 是被驅動端 Adapters。
...
- [BEE-5001](monolith-vs-microservices-vs-modular-monolith.md)：架構模式概覽
- [BEE-5002](domain-driven-design-essentials.md)：領域驅動設計
