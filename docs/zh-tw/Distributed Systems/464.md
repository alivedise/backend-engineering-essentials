...
- [BEE-124](../Data Storage and Database Fundamentals/124.md) -- 儲存引擎：涵蓋 LSM Tree 和 B-Tree，兩者均為列式儲存引擎；欄式儲存是針對分析讀取模式最佳化的第三種模型
- [BEE-143](../Data Modeling and Schema Design/143.md) -- 編碼與序列化格式：涵蓋 Avro、Protocol Buffers 和 Thrift；Parquet 和 Arrow 是適用於分析工作負載的欄式等價格式
- [BEE-121](../Data Storage and Database Fundamentals/121.md) -- 索引深度解析：欄式儲存使用基於統計資訊的跳過（每列群組 min/max），而非傳統 B-Tree 索引；了解兩者有助於選擇正確的工具
- [BEE-303](../Performance and Scalability/303.md) -- 效能分析與瓶頸識別：I/O 讀取放大是欄式儲存消除的主要瓶頸；在遷移前使用分析工具確認
