...
- [BEE-144](../Data Modeling/144.md) -- 設計時間序列和稽核資料：涵蓋僅附加事件資料表和稽核日誌；時態資料建模是互補的——兩者都保留歷史，但時態資料表針對時間點查詢進行優化
- [BEE-142](../Data Modeling/142.md) -- 綱要演進和向後相容性：向現有資料表添加有效時間欄位是一種綱要演進；valid_to 的哨兵值必須在插入第一列之前選擇
- [BEE-126](../Databases/126.md) -- 資料庫遷移：時態資料表需要謹慎的遷移策略；追溯地向現有資料表添加 valid_from/valid_to 需要用歷史資料或預設時間點回填
