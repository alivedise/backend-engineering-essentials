...
- [BEE-143](../Data Modeling and Schema Design/143.md) -- 編碼與序列化格式：在比較層面涵蓋 Avro、Protocol Buffers、Thrift 和 MessagePack；本文提供在生產環境中安全演進 protobuf Schema 所需的深度
- [BEE-142](../Data Modeling and Schema Design/142.md) -- Schema 演進與向後相容性：通用原則（向後相容性、向前相容性、required vs optional）同樣適用於 protobuf、Avro 和 SQL Schema
- [BEE-465](465.md) -- gRPC 串流模式：gRPC 使用 protobuf 作為其 IDL 和線路格式；Schema 演進決定哪些 proto 變更需要新的 gRPC 主要版本，哪些可以透過滾動升級部署
- [BEE-126](../Data Storage and Database Fundamentals/126.md) -- 資料庫遷移：protobuf Schema 演進與 SQL Schema 遷移共享相同約束——變更必須可在不同步切換所有生產者和消費者的情況下部署
