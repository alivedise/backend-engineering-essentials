...
- [BEE-420](420.md) -- CAP 定理：W+R>N 的仲裁是 CP（仲裁不可用時拒絕請求）；鬆散仲裁是 AP（路由到任何可用節點）；NWR 是 CAP 所描述的一致性-可用性取捨的一種實現機制
- [BEE-421](421.md) -- 共識演算法：Raft 和 Paxos 對所有操作使用嚴格多數仲裁（N/2+1），並通過全序排列保證線性化——這比單獨的 NWR 仲裁更強，後者只保證仲裁交集，不保證單一序列化順序
- [BEE-422](422.md) -- 向量時鐘與邏輯時間戳：Dynamo 使用向量時鐘檢測仲裁交集無法解決的並發寫入；當兩次寫入並發到達重疊仲裁時，向量時鐘識別衝突，讓應用程序（或 LWW）解決
- [BEE-432](432.md) -- 默克爾樹：Dynamo 和 Cassandra 使用默克爾樹進行反熵修復——鬆散仲裁寫入後，後台修復使用默克爾樹比較找到在分區期間錯過寫入的副本並同步它們
