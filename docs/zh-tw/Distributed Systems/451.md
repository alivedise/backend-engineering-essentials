...
- [BEE-261](../Resilience and Reliability/261.md) -- 重試策略與指數退避：對沖請求和重試是互補的；重試容忍完全失敗，對沖請求容忍慢速回應；對沖 MUST（必須）像重試一樣受到速率限制
- [BEE-262](../Resilience and Reliability/262.md) -- 逾時與截止時間：對沖延遲是觸發並行嘗試而非失敗的軟逾時；外部請求上的硬逾時仍然限制總延遲
- [BEE-225](../Messaging and Event-Driven/225.md) -- 背壓與流量控制：有界佇列和佇列滿時快速失敗是客戶端對沖的伺服器端補充；容尾系統需要兩者
- [BEE-303](../Performance and Scalability/303.md) -- 效能分析與瓶頸識別：尾延遲根本原因分析需要火焰圖、GC 暫停指標和每實例延遲直方圖，而非聚合平均值
- [BEE-260](../Resilience and Reliability/260.md) -- 熔斷器模式：當對沖單獨無法補償降級節點時，對持續緩慢副本的熔斷器是升級路徑
