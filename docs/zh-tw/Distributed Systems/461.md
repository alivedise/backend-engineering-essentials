...
- [BEE-125](../Data Storage and Database Fundamentals/125.md) -- 連線池與查詢最佳化：N+1 查詢會成倍增加連線使用量；一個端點中的 N+1 耗盡的連線池會降低所有其他端點的效能
- [BEE-121](../Data Storage and Database Fundamentals/121.md) -- 索引深度解析：N+1 查詢各自單獨訪問索引；即使是快速的索引查找，在重複 N 次後也會變慢；批次載入分攤了索引遍歷成本
- [BEE-74](../API Design and Communication Protocols/74.md) -- GraphQL vs REST vs gRPC：GraphQL 的每欄位解析器模型是 GraphQL 中 N+1 的結構性原因；DataLoader 是標準的緩解措施
- [BEE-200](../Caching/200.md) -- 快取基礎與快取階層：DataLoader 的每請求快取是請求內去重的最小、正確的快取；它補充但不取代持久快取層
