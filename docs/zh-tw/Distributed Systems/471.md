...
- [BEE-163](../Transactions and Consistency/163.md) -- Saga 模式：Saga 是需要編排或協調來協調跨服務補償交易的分散式交易模式
- [BEE-221](../Messaging and Event-Driven/221.md) -- 發佈-訂閱模式：編排建立在發佈訂閱之上；理解扇出、消費者群組和事件排序是基於編排工作流程的前提
- [BEE-102](../Architecture Patterns/102.md) -- CQRS：協調者非常適合作為 CQRS 命令端；協調者處理命令並產生更新讀取模型的事件
- [BEE-222](../Messaging and Event-Driven/222.md) -- 交付保證：在編排中，事件交付保證（至少一次、精確一次）決定了事件處理器是否必須是冪等的——它們必須是
