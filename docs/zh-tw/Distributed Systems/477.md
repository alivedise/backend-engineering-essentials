...
- [BEE-120](../Databases/120.md) -- SQL vs NoSQL 權衡：JSONB 縮小了與文件儲存之間的許多差距而不脫離關聯式模型；了解何時專用文件資料庫仍是正確選擇
- [BEE-121](../Databases/121.md) -- 索引深入探討：GIN 是 PostgreSQL 的專業索引型別之一；了解它與 B-tree 的差異以及各自適用的時機
- [BEE-401](../Multi-Tenancy/401.md) -- 租戶隔離策略：JSONB `metadata` 欄位通常用於租戶特定屬性；生成欄位和函數索引使 tenant_id 可查詢而不需要綱要更改
