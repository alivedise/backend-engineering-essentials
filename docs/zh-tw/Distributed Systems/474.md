...
- [BEE-76](../API Design and Communication Protocols/76.md) -- Webhook 與回呼模式：涵蓋 Webhook 架構設計（重試策略、交付排序、扇出）；本文涵蓋每個 Webhook 接收者都必須實作的安全層
- [BEE-32](../Security Fundamentals/32.md) -- 機密管理：共享的 HMAC 密鑰必須使用與其他對稱金鑰相同的實踐來儲存和輪換——環境變數、機密管理器、絕不放入原始碼
- [BEE-34](../Security Fundamentals/34.md) -- 工程師的密碼學基礎：HMAC-SHA256 是一個金鑰雜湊函數；理解雜湊特性（抗碰撞性、單向性）可以解釋為何簽名驗證能提供其所聲稱的保證
