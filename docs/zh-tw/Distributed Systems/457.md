...
- [BEE-424](424.md) -- 分散式鎖定：排程器觸發層的 leader 選舉機制是分散式鎖定原語的直接應用
- [BEE-447](447.md) -- Fencing Token：在使用分散式鎖進行排程器 leader 選舉時，fencing token 防止過期的 leader 在其鎖過期後插入重複任務
- [BEE-436](436.md) -- Lease 協調：任務工作者的心跳/lease 模式與 BEE-436 中描述的 lease 原語相同
- [BEE-224](../Messaging and Event-Driven/224.md) -- 死信佇列與毒訊息：超過重試限制的任務是任務佇列中毒訊息的等效物；DLQ 模式直接適用
- [BEE-164](../Transactions and Consistency/164.md) -- 冪等性與精確一次語義：所有排程任務必須（MUST）設計為冪等性，以安全容忍至少一次傳遞
- [BEE-427](427.md) -- 時鐘同步與物理時間：cron 表達式根據掛鐘時間評估；跨副本的時鐘偏差是重複和遺漏任務觸發的根本原因
