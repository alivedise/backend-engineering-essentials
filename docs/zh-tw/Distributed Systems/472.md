...
- [BEE-437](437.md) -- 變更資料擷取（CDC）：CDC（Debezium、基於 WAL）是高吞吐量下 Outbox 模式的推薦中繼機制；outbox 資料表是 CDC 來源
- [BEE-226](../Messaging and Event-Driven/226.md) -- 冪等訊息處理：Outbox 模式提供至少一次交付；消費者必須實作冪等性以處理重複事件
- [BEE-163](../Transactions and Consistency/163.md) -- Saga 模式：Outbox 模式是以原子方式發佈 Saga 步驟事件與對應資料庫狀態變更的標準實作機制
- [BEE-471](471.md) -- 分散式工作流程中的編排 vs 協調：在基於編排的 Saga 中，每個服務透過 Outbox 模式發佈事件，以保證事件永遠不會被靜默遺失
