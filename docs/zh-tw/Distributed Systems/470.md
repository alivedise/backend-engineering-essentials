...
- [BEE-121](../Data Storage and Database Fundamentals/121.md) -- 索引深度解析：`REFRESH CONCURRENTLY` 需要唯一索引；物化視圖存取模式的索引選擇遵循與基底資料表相同的原則
- [BEE-200](../Caching/200.md) -- 快取基礎與快取層次：物化視圖是資料庫層的快取；理解快取層次有助於釐清何時使用物化視圖而非應用層快取
- [BEE-102](../System Architecture/102.md) -- CQRS：物化視圖是 CQRS 中讀取模型的常見實作方式——寫入模型更新來源資料表，讀取模型是從這些資料表重整的物化視圖
- [BEE-437](437.md) -- 變更資料擷取（CDC）：CDC 是為增量計算系統提供資料的機制；基於 WAL 的 CDC 讓串流處理器只需消費來源資料表中已變更的資料列
