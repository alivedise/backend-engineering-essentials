...
- [BEE-165](../Transactions/165.md) -- 最終一致性模式：CRDT 是最終一致系統的原則性實現機制——它們精確指定了分散的副本如何收斂
- [BEE-420](420.md) -- CAP 定理：CRDT 是 AP 策略——它們為了可用性和分區容忍性犧牲強一致性（無共識、無線性化讀取）；收斂是它們的一致性替代
- [BEE-423](423.md) -- 流言協議：CRDT 狀態傳播通常使用流言——節點定期交換狀態（或增量），CRDT 合並處理收斂，無論流言順序如何
- [BEE-222](../Messaging/222.md) -- 投遞保證：基於操作的 CRDT（CmRDT）需要操作的恰好一次因果投遞；基於狀態的 CRDT（CvRDT）只需要完整狀態的最終投遞
