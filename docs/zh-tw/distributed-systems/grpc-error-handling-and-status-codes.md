...
- [BEE-4005](../api-design/graphql-vs-rest-vs-grpc.md) -- GraphQL vs REST vs gRPC：涵蓋何時選擇 gRPC 而非 REST；錯誤處理是 gRPC 的差異化優勢之一
- [BEE-12002](../resilience/retry-strategies-and-exponential-backoff.md) -- 重試策略與指數退避：通用重試理論；gRPC 服務設定在通道層實作了這一點
- [BEE-12003](../resilience/timeouts-and-deadlines.md) -- 逾時與截止時間：截止時間傳播與 `DEADLINE_EXCEEDED` 互動；了解截止時間與狀態碼之間的關係
- [BEE-19046](grpc-streaming-patterns.md) -- gRPC 串流模式：串流 RPC 使用相同的狀態碼模型；串流以單一尾隨狀態終止
