...
**必須（MUST）將 Webhook 密鑰作為應用程式機密儲存，而非存放在原始碼或版本控制中。** Webhook 密鑰是一個對稱金鑰；洩露意味著攻擊者可以偽造任何負載。將其存儲在與其他機密相同的系統中（BEE-32）：環境變數、機密管理器（AWS Secrets Manager、HashiCorp Vault）或加密的配置。
