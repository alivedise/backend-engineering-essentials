...
**流言是最終傳播的正確工具，而非強一致性的工具。** 它*最終*將狀態傳遞給所有節點——收斂是概率性的，需要 O(log N) 輪。在這些輪次中，節點持有不同的叢集狀態視圖。這對於成員資格信息（知道節點加入或離開可以滯後一秒）、故障檢測（在將節點標記為失敗之前有短暫的不確定窗口）和配置傳播（配置值的滾動更新）是可接受的。對於需要所有節點在同一時刻就一個值達成一致的協調決策，則不可接受——使用共識（BEE-19002）。
...
- [BEE-19002](consensus-algorithms-paxos-and-raft.md) -- 共識演算法：Raft 使用以領導者為中心的日誌複製模型，需要法定人數；流言是去中心化的，不需要法定人數——針對不同問題的互補工具
- [BEE-19001](cap-theorem-and-the-consistency-availability-tradeoff.md) -- CAP 定理：基於流言的系統是 AP 的——它們在分區期間保持可用，但分區兩側的節點會分歧，直到分區癒合
- [BEE-6003](../data-storage/replication-strategies.md) -- 複製策略：流言傳播叢集元數據（誰擁有哪個分區）；複製策略本身是在此之上的獨立層
- [BEE-12006](../resilience/chaos-engineering-principles.md) -- 混沌工程：向流言叢集注入隨機節點故障並測量檢測延遲，是對故障檢測調整的直接測試
