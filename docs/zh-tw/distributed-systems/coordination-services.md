...
**臨時節點和租約是存活機制。** 領導者選舉與分散式鎖的運作方式是把資源所有權綁定在工作階段的健康狀態上。在 ZooKeeper 中，臨時 znode 在建立它的客戶端工作階段到期時自動刪除；工作階段到期即是崩潰的鎖定持有者釋放鎖的機制。在 etcd 中，租約是客戶端必須定期續期的有效期令牌；附加到租約的鍵在租約未續期時到期。兩種機制都要求應用程式處理工作階段遺失或租約到期事件，並可能重新獲取協調原語。
...
- [BEE-19002](consensus-algorithms-paxos-and-raft.md) -- Consensus Algorithms: Paxos and Raft：協調服務是共識狀態機——ZooKeeper 使用 Zab（Paxos 的變體），etcd 使用 Raft；協調服務的可線性化保證完全來自底層的共識協定
- [BEE-19005](distributed-locking.md) -- Distributed Locking：分散式鎖是在協調服務原語之上構建的核心配方之一（ZooKeeper 中的循序臨時 znode，etcd 中的租約支援鍵）；協調服務提供原始 Redis 或資料庫鎖所無法提供的持久性和故障偵測
- [BEE-19017](lease-based-coordination.md) -- Lease-Based Coordination：租約是 etcd 的臨時狀態主要機制；理解租約續期、TTL 選擇和租約到期處理是在 etcd 上構建可靠原語的前提
- [BEE-19007](service-discovery.md) -- Service Discovery：服務實例將其地址以臨時 znode 或租約支援鍵的形式在協調服務中註冊；協調服務故障會使服務發現暫時不可用，將它們的可用性耦合在一起
- [BEE-19028](fencing-tokens.md) -- Fencing Tokens：etcd 鎖定獲取時返回的版本號，或 ZooKeeper 的 zxid，作為下游資源用來拒絕過期鎖定持有者的隔離令牌
