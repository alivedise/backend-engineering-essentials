...
Leslie Lamport 在 1978 年的「分散式系統中事件的時間、時鐘和排序」（Communications of the ACM，1978 年 7 月）中建立了理論基礎。他的核心觀察：分散式系統不能依賴同步的物理時鐘來確定事件排序，因為進程沒有共享的時間概念。他提出的解決方案——基於 happens-before 關係的邏輯時鐘——完全繞開了物理時間（完整處理見 BEE-19003）。但邏輯時鐘不攜帶任何掛鐘時間信息，當您需要回答「這個系統在 UTC 14:00:00 的狀態是什麼？」或「這次提交是在那次之前還是之後？」這類問題時，物理時間就很重要了。
