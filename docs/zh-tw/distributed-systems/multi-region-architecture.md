...
- [BEE-19001](cap-theorem-and-the-consistency-availability-tradeoff.md) -- CAP 定理與一致性-可用性權衡：多區域主動-主動從根本上是選擇偏好可用性和分區容錯性而非強一致性
- [BEE-19010](crdts-conflict-free-replicated-data-types.md) -- CRDT：LWW 資料丟失不可接受的主動-主動寫入模式的正確無衝突資料結構
- [BEE-19018](change-data-capture.md) -- 變更資料捕獲：Aurora Global Database、DynamoDB Global Tables 等系統用於跨區域非同步複製寫入的機制
- [BEE-19003](vector-clocks-and-logical-timestamps.md) -- 向量時鐘與邏輯時間戳：讀取自己的寫入一致性的因果令牌是向量時鐘概念的實際應用
- [BEE-8006](../transactions/eventual-consistency-patterns.md) -- 最終一致性模式：帶非同步延遲的主動-被動複製是實踐中主要的最終一致性模式之一
