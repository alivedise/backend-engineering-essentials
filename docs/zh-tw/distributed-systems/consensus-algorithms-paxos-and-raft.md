...
**少數分區最多只能讀取。** 當網路分區將少數節點隔離時，這些節點無法選舉領導者（它們缺乏法定人數）。它們無法提交寫入。如果它們之前是跟隨者，它們最終會讓選舉超時到期並反复嘗試形成法定人數——它們沒有進展。多數分區選舉新的領導者並繼續服務。當分區癒合時，少數節點從新領導者的日誌追上，丟棄它們持有的任何未提交條目。這是 CP 行為（BEE-19001）：系統犧牲少數分區的可用性來維護一致性。
