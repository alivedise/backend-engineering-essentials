...
- [BEE-7005](../data-modeling/designing-for-time-series-and-audit-data.md) -- 時序與審計資料設計：軟刪除的審計日誌替代方案使用具有相同設計原則的只追加事件資料表
- [BEE-19018](change-data-capture.md) -- 變更資料擷取：軟刪除在 CDC 管道中產生 UPDATE 事件；物理清除產生 tombstone 事件；下游消費者必須處理兩者
- [BEE-19038](distributed-job-scheduling.md) -- 分散式任務排程：每晚清除任務和刪除請求履行任務是需要冪等性和死信處理的計劃後台任務的典型例子
- [BEE-6007](../data-storage/database-migrations.md) -- 資料庫遷移：向現有資料表添加 `deleted_at` 欄位是一個零停機相容的遷移（可空欄位，無預設值——在所有主要資料庫中都是安全的）
- [BEE-19039](zero-downtime-schema-migrations.md) -- 零停機 Schema 遷移：在大型生產資料表上將硬刪除資料表轉換為軟刪除時，展開-收縮模式適用
