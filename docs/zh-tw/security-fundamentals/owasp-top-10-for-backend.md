...
後端相關性：存取控制必須在每個請求的伺服器端強制執行。參見 [BEE-1001](../auth/authentication-vs-authorization.md)。
...
後端相關性：靜態資料（data-at-rest）與傳輸中資料（data-in-transit）的每個決策都涉及加密。參見 [BEE-2005](cryptographic-basics-for-engineers.md)。
...
後端相關性：注入攻擊鎖定資料層，以及任何從輸入建構命令字串的系統呼叫。參見 [BEE-2002](input-validation-and-sanitization.md)。
...
後端相關性：設定必須按環境稽核；正式環境（production）與開發環境（development）的設定必須有所不同。CORS 細節參見 [BEE-2004](cors-and-same-origin-policy.md)。
...
後端相關性：相依套件清單（dependency inventory）、自動漏洞掃描與及時修補，是不可忽視的必要工作。參見 [BEE-2006](dependency-security-and-supply-chain.md)。
...
- [BEE-10: Authentication vs Authorization](10.md) — 基礎存取控制流水線（A01、A07）
- [BEE-31: Input Validation and Sanitization](31.md) — 防禦注入攻擊（A03）
- [BEE-32: Secrets Management](32.md) — 保護憑證與金鑰（A02）
- [BEE-33: CORS and Same-Origin Policy](33.md) — 瀏覽器端存取控制設定（A05）
- [BEE-34: Cryptographic Basics for Engineers](34.md) — 加密與雜湊基礎（A02）
- [BEE-35: Dependency Security and Supply Chain](35.md) — 管理第三方風險（A06）
