...
- **BEP-200** -- 快取基礎：快取模式（cache-aside、write-through、write-behind）與快取時機。
- **BEP-201** -- 快取驅逐策略：LRU、LFU 與容量限制如何與失效互動。
- **BEP-201** -- 快取雪崩與 Thundering Herd：處理失效後引發的大量未命中。
- **BEP-220** -- 訊息傳遞模式：驅動事件驅動失效的發布/訂閱基礎設施。
