...
在 IaC 的 CI/CD 中，計畫步驟是強制性的，其輸出會附加到 Pull Request 供審查者檢查。只有在計畫被審查且 PR 被批准後才執行套用。參見 [BEP-360：IaC 的 CI](#) 了解流水線模式。
...
- [BEP-32：機密管理](#) — 密鑰存儲模式、輪換和執行時注入
- [BEP-360：IaC 的 CI](#) — 流水線結構：lint、計畫、政策檢查、套用閘道
- [BEP-361：部署策略](#) — 不可變基礎設施如何實現藍綠和金絲雀部署
