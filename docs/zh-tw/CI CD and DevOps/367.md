...
- [BEE-360](360.md) -- 持續整合原則：CI 產生 GitOps 交付的成品（容器映像）
- [BEE-361](361.md) -- 部署策略：GitOps 是機制；藍綠、金絲雀和滾動是它執行的策略
- [BEE-362](362.md) -- 基礎設施即程式碼：GitOps 將相同的 Git 作為真相來源的原則應用於即時調和而非一次性套用
- [BEE-363](363.md) -- 功能旗標：使用 Argo Rollouts/Flagger 的漸進式交付補充功能旗標以進行生產流量切換
- [BEE-366](366.md) -- 灰度與暗發布：Argo Rollouts 實現了其中描述的金絲雀和藍綠模式
