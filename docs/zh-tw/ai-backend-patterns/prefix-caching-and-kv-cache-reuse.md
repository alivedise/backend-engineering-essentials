...
**期望前綴快取能加速 token 生成。** 前綴快取只加速預填充階段（TTFT）。解碼階段——生成每個輸出 token——不受影響。對於提示短、輸出長的工作負載（例如故事生成、代碼生成），收益很小。使用負載測試（BEE-560）測量您特定工作負載的實際 TTFT 對比 TPOT。
...
**將前綴快取用作語義快取的替代品。** 前綴快取需要精確的 token 級匹配。語義上相同但措辭不同的提示（「您是有幫助的」對比「請提供幫助」）會錯過快取。對於近似重複的提示去重，語義快取（BEE-558）是互補工具。
