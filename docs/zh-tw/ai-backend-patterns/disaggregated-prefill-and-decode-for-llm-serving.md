...
在同一 GPU 上同時運行兩個階段會導致結構性相互干擾。一個大型預填充請求會阻塞其他序列的解碼步驟（隊首阻塞），使 TTFT 飆升。解碼密集型工作負載讓 GPU 長期處於記憶體頻寬受限的模式，剝奪了預填充所需的計算能力。分塊預填充（BEE-567）能緩解最嚴重的 TTFT 尖峰，但無法消除根本性的資源爭用問題。
