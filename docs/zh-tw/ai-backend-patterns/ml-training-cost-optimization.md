...
- [BEE-590 ML 模型服務基礎設施](590) — 具有動態批次的推論成本最佳化與訓練成本最佳化互補
- [BEE-586 ML 實驗追蹤與模型登錄庫](586) — 將每次運行的成本指標與模型品質一起記錄到 MLflow，進行成本/準確率權衡分析
- [BEE-572 大型語言模型的分散式訓練基礎設施](572) — LLM 規模訓練成本的 DeepSpeed ZeRO 和張量並行
- [BEE-303 性能分析與瓶頸識別](303) — 適用於 GPU 訓練性能分析的一般性能分析原則
