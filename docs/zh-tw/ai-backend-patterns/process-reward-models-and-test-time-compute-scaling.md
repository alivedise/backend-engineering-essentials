...
- [BEE-30071](rlhf-and-alignment-training-infrastructure.md) -- RLHF 與對齊訓練基礎設施：GRPO 之於 RLHF，如同 DPO 之於 PPO——更簡單、更高效的替代方案；兩者都屬於對齊訓練家族
- [BEE-30023](chain-of-thought-and-extended-thinking-patterns.md) -- 思維鏈與延伸思考模式：思維鏈是推理機制；測試時計算縮放是分配更多推理預算的基礎設施策略
- [BEE-30059](speculative-decoding-for-llm-inference.md) -- LLM 推論的投機解碼：投機解碼加速推理模型的解碼階段，降低長思考鏈的實際延遲
- [BEE-30067](disaggregated-prefill-and-decode-for-llm-serving.md) -- LLM 服務的預填充與解碼分離：推理模型的長推理 token 生成是分離架構所解決的解碼瓶頸的極端案例
- [BEE-30041](llm-self-consistency-and-ensemble-sampling.md) -- LLM 自洽性與集成採樣：自洽性（多數投票）是使用一致性而非訓練驗證器的 Best-of-N 形式
