...
單模態嵌入向量（BEE-30014）對單一輸入類型操作：文字對文字的相似性需要文字嵌入，圖片對圖片的相似性需要圖片嵌入。跨模態檢索 — 從文字查詢找到圖片，或從圖片找到文字 — 需要一個共享嵌入空間，使不同模態的表示在幾何上對齊。
...
- [BEE-30014](embedding-models-and-vector-representations.md) -- 嵌入模型與向量表示：單模態嵌入基礎知識
- [BEE-30026](vector-database-architecture.md) -- 向量資料庫架構：存儲和查詢嵌入索引
- [BEE-30019](multi-modal-llm-integration-patterns.md) -- 多模態 LLM 整合模式：在檢索旁邊使用多模態 LLM
