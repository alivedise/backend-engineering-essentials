...
- [BEE-30007](rag-pipeline-architecture.md) -- RAG 管線架構：此處描述的兩階段檢索管線是 RAG 系統的生產級檢索元件
- [BEE-30014](embedding-models-and-vector-representations.md) -- 嵌入模型與向量表示：雙編碼器嵌入模型驅動稠密檢索階段；模型選擇和量化決策直接適用
- [BEE-17004](../search/vector-search-and-semantic-search.md) -- 向量搜尋與語義搜尋：ANN 索引結構（HNSW、IVF）和近似搜尋是稠密檢索階段的底層機制
- [BEE-30010](llm-context-window-management.md) -- LLM 上下文視窗管理：重排序器的前 k 輸出必須符合分配的 RAG 上下文預算
