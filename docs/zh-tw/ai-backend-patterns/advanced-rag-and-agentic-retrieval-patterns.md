...
基本 RAG（BEE-509）建立了一條流程：分塊文件、嵌入、儲存於向量資料庫、嵌入查詢、以餘弦相似度檢索前 k 個、注入上下文、生成。這條流程對結構良好語料庫的窄域事實查找效果良好，但在四種可識別的情境下會退化。
...
2. **檢索**——核心向量/關鍵字/混合搜尋（BEE-517）
3. **檢索評估與修正**（CRAG）——驗證所檢索的內容是否相關
4. **重排序**（BEE-517）——以交叉編碼器重新排序檢索文件
