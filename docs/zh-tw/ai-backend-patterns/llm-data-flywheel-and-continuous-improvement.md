...
**評估**：線上評估以非同步方式在生產軌跡樣本上執行，產生品質分數。這不同於黃金資料集的離線評估（BEE-506）——線上評估在真實使用者查詢、真實生產條件下運作，揭露精選資料集從未預期的失敗模式。
...
**必須（MUST）** 使用與被評估模型不同系列的模型作為評估器。自我評估呈現自我偏好偏差——Claude 模型對 Claude 輸出評分更高（請見 BEE-536 了解跨系列評審模式）。
...
**必須（MUST）** 透過與任何生產變更相同的分段發佈流程部署所有改善——無論是提示詞更新還是微調後的模型權重：影子模式驗證後接金絲雀遞增（請見 BEE-536）。飛輪的價值在於複利迭代；每次改善在下一個週期開始前必須經過驗證：
...
兩者都要使用：離線評估把關部署（BEE-506）；線上評估驅動下一個改善週期。
