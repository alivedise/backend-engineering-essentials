...
- [BEE-5002: Domain-Driven Design](../architecture-patterns/domain-driven-design-essentials.md) — 限界上下文 (Bounded Context) 與聚合 (Aggregate) 決定哪些實體屬於同一個模型。
- [BEE-6001: SQL vs NoSQL](../data-storage/sql-vs-nosql-tradeoffs.md) — ER 模型適用於關聯式資料庫；文件型儲存需要不同的建模策略。
- [BEE-7002: 正規化](normalization-and-denormalization.md) — 將 ER 轉換為資料表後，正規化規則進一步精化實體設計。
- [BEE-7006: Schema 設計中的多型](polymorphism-in-data-models.md) — 當實體有子類型時，ER 建模必須在每類型一表、每繼承層級一表等策略間做出決定。
