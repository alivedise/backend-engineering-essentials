...
出現在 `WHERE`、`JOIN ON` 和 `ORDER BY` 子句中的欄位都是索引的候選。詳見 [BEE-121](121.md)。
...
- [BEE-121](121.md) — 資料庫索引策略：哪些欄位要建索引以及如何建
- [BEE-301](301.md) — 資源管理：連線生命週期與清理模式
- [BEE-303](303.md) — 效能分析與可觀測性：在生產環境中偵測慢查詢與 N+1
