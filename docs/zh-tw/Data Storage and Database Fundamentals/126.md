...
:::tip 深入探討
關於資料庫層級的結構演進模式和向後相容性，請見 [DDP Schema Evolution 系列](https://alivedise.github.io/database-design-principles/300)。
:::
...
- **[BEP-104] 絞殺者無花果模式** — 相同的增量、平行操作哲學應用於服務拆分；擴展-收縮是其資料庫對應版本。
- **[BEP-142] 結構演進與向後相容** — 更深入探討服務間的結構版本控制契約。
- **[BEP-361] 部署策略** — 滾動部署、藍綠部署和金絲雀模式，與遷移順序的互動。
