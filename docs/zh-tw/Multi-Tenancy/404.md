...
- [BEE-126](126.md) -- 資料庫遷移：通用遷移實踐；本文將其擴展到多租戶場景
- [BEE-400](400.md) -- 多租戶架構模型：決定是否需要扇出的孤島/共享池/橋接模型
- [BEE-403](403.md) -- 租戶入職與配置流水線：使用快照而非完整歷史重播的新租戶 schema 初始化
- [BEE-361](361.md) -- 部署策略：擴展-收縮需要協調的應用程式和 schema 部署排序
