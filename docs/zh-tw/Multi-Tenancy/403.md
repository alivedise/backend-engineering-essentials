...
- [BEE-400](400.md) -- 多租戶架構模型：孤島與共享池模型決定了配置流水線的形狀
- [BEE-401](401.md) -- 租戶隔離策略：入職期間配置的隔離策略（RLS、schema、命名空間）
- [BEE-163](163.md) -- Saga 模式：具有失敗補償事務的多步驟配置
- [BEE-164](164.md) -- 冪等性與精確一次語義：冪等配置步驟使安全重試成為可能
