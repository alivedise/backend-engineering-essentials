...
- Backend 前的 TLS 終止（見 [BEE-3004](tls-ssl-handshake.md)）
- WebSocket upgrade 與 HTTP/2 多路複用（見 [BEE-3003](http-versions.md)）
...
- [BEE-3001](tcp-ip-and-the-network-stack.md) — TCP/IP 與網路堆疊：L4 負載平衡器在 TCP 層運作；理解 TCP 連線狀態（SYN、ESTABLISHED、TIME_WAIT）是推理 connection draining 和健康檢查行為的必要條件
- [BEE-3002](dns-resolution.md) — DNS 解析：DNS-based 負載平衡（多個 A 記錄、GeoDNS）是互補技術；另見 DNS TTL 如何影響 client 偵測失敗 backend 的速度
- [BEE-3003](http-versions.md) — HTTP/2 與 HTTP/3：HTTP/2 在一條 TCP 連線上多路複用多個 stream；L7 LB 必須理解多路複用，才能將一條連線的工作分配到多個 backend，而非將所有 stream 固定到一個 backend
- [BEE-3004](tls-ssl-handshake.md) — TLS/SSL Handshake：L7 負載平衡器是標準的 TLS 終止點；見 TLS 終止拓撲（邊緣終止、重加密、直通）以及 SNI 如何在 TLS handshake 完成前就實現路由
- [BEE-3006](proxies-and-reverse-proxies.md) — Reverse Proxy：reverse proxy 和 L7 負載平衡器有大量重疊；[BEE-3006](proxies-and-reverse-proxies.md) 涵蓋支撐 L7 負載平衡行為的 proxy 語義、請求緩衝與上游連線池
