...
- [BEE-3001 — TCP/IP and the Network Stack](tcp-ip-and-the-network-stack.md)：TCP 握手、慢啟動，以及為何 HTTP/3 以 QUIC 取代 TCP。
- [BEE-3004 — TLS/SSL Handshake](tls-ssl-handshake.md)：ALPN 擴充、TLS 1.3 與 QUIC 的整合、憑證協商。
- [BEE-3005 — Load Balancers](load-balancers.md)：HTTP/2 與 HTTP/3 的第 7 層終止，對上游的協定翻譯。
- [BEE-4001 — REST API Design](../api-design/rest-api-design-principles.md)：協定版本如何影響 API 延遲與用戶端連線策略。
- [BEE-9006 — HTTP Caching](../caching/http-caching-and-conditional-requests.md)：Cache-Control 語意定義於 RFC 9110，在所有 HTTP 版本中完全相同。
- [BEE-3008 — HTTP as a Substrate for New Protocols](http-as-a-substrate-for-new-protocols.md)：說明 SSE、WebSocket、gRPC、Streamable HTTP 與 WebTransport 如何疊加在這些版本之上，以及各自倚賴哪個版本的原語。
