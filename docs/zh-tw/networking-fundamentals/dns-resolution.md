...
- [BEE-50 — TCP/IP and the Network Stack](./50.md)：DNS 解析後發生的事情——TCP 連線生命週期。
- [BEE-53 — TLS/SSL Handshake](./53.md)：TLS 在 TCP 之上增加了另一次往返；DNS + TCP + TLS 意味著在第一個 HTTP 位元組之前需要三次往返。
- [BEE-51 — Load Balancers](./51.md)：L7 負載平衡器作為 DNS 負載平衡的替代方案，具備健康檢查能力。
- [BEE-261 — Timeouts](261.md)：DNS 解析是總請求延遲的一部分；明確設定 DNS 逾時預算。
