...
- [BEE-3001 — TCP/IP and the Network Stack](tcp-ip-and-the-network-stack.md)：DNS 解析後發生的事情——TCP 連線生命週期。
- [BEE-3004 — TLS/SSL Handshake](tls-ssl-handshake.md)：TLS 在 TCP 之上增加了另一次往返；DNS + TCP + TLS 意味著在第一個 HTTP 位元組之前需要三次往返。
- [BEE-3002 — Load Balancers](dns-resolution.md)：L7 負載平衡器作為 DNS 負載平衡的替代方案，具備健康檢查能力。
- [BEE-12002 — Timeouts](../resilience/retry-strategies-and-exponential-backoff.md)：DNS 解析是總請求延遲的一部分；明確設定 DNS 逾時預算。
