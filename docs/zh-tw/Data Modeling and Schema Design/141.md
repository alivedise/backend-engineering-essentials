...
:::tip 深入閱讀
關於資料庫層級的正規化理論與正式定義，請參閱 [DDP-100: 正規化](https://alivedise.github.io/database-design-principles/100)。
:::
...
對於複雜的讀取工作負載，可考慮將寫入模型（正規化）與讀取模型（反正規化、為查詢優化）分開。詳細說明請參閱 [BEP-102: CQRS](./102.md)。
...
- [BEP-102: CQRS](./102.md) — 將讀取模型分離，作為一種結構化的反正規化模式
- [BEP-120: SQL vs NoSQL](./120.md) — 文件型資料庫在反正規化資料上的適用場景
- [BEP-140: Entity-Relationship 建模](./140.md) — 正規化前如何識別實體
