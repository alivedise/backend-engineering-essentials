...
**適用時機：** 子型別屬性高度可變且頻繁演進、需要不經過遷移就能彈性調整結構描述，或已處於文件儲存的情境（[BEE-120](120.md)）。也適用於子型別變體數量龐大且難以預先定義的情況。
...
- [BEE-120](120.md) — SQL vs NoSQL：文件儲存原生處理多型資料，不需要此處描述的任何關聯式繼承模式。
- [BEE-140](140.md) — ER 建模：繼承映射的決策直接源於實體關係分析。
- [BEE-141](141.md) — 正規化：類別表繼承是正規化的方式；單表繼承則為了查詢簡潔而刻意反正規化。
