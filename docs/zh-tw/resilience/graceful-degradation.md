...
明確記錄此層級清單。它將指導斷路器備援方案（[BEE-12001](circuit-breaker-pattern.md)）、艙壁分區大小（[BEE-12004](bulkhead-pattern.md)）以及待命值班的分流決策。
...
功能旗標（見 [BEE-16004](../cicd-devops/feature-flags.md)）是優雅降級的操作槓桿。相較於需要重新部署才能在高負載下停用非關鍵功能，關閉開關旗標讓維運人員可以在幾秒內停用功能。
...
- 商品資訊：正常回傳。
- 庫存數量：正常回傳。
- 評論：斷路器（[BEE-12001](circuit-breaker-pattern.md)）開路；觸發備援；評論區塊從回應中省略。回應 metadata 加入 `_degraded: ["reviews"]`。
- 推薦：正常回傳。
...
優雅關機（[BEE-278](278.md)）確保部署或重啟期間進行中的請求被完整完成或安全移交。優雅降級確保運行中的服務在局部依賴故障時不會引發級聯失敗。兩者都是必要的，互不替代。
...
備援程式碼路徑在正常運作中很少被執行。備援邏輯中的錯誤只在真實事故中才會被發現——恰好是系統已經承受壓力的時候。在整合測試和混沌工程演練中（[BEE-12006](chaos-engineering-principles.md)）測試每條備援路徑。模擬評論服務停機；確認評論區塊是缺席的，而不是損壞的。
...
- [BEE-12001](circuit-breaker-pattern.md)（斷路器模式）——斷路器在依賴故障時觸發降級；[BEE-12005](graceful-degradation.md) 定義備援方案的內容
- [BEE-12004](bulkhead-pattern.md)（艙壁模式）——艙壁按依賴隔離資源；當分區滿載時，優雅降級決定回傳什麼
- [BEE-12007](rate-limiting-and-throttling.md)（速率限制與節流）——速率限制作為入站流量的負載削減形式
- [BEE-16004](../cicd-devops/feature-flags.md)（功能旗標）——降級期間用於停用非關鍵功能的即時關閉開關
