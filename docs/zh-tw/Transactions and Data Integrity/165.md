...
寫入提交後到該寫入對所有副本可見之間的時間差稱為**一致性視窗**。監控複製延遲（參見 BEE-122）。如果視窗超過你的 SLA 則發出警報。在可觀測性儀表板中公開複製延遲。
...
- **BEE-122** -- 複製延遲：測量和警報一致性視窗
- **BEE-160** -- ACID 事務：何時改用強一致性
- **BEE-162** -- 分散式事務：跨服務協調寫入
- **BEE-203** -- 分散式快取：快取失效與資料陳舊
