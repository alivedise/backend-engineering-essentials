...
**選擇正確的隔離層級是應用程式設計決策**，而非一個可以盲目接受的資料庫預設值。完整的隔離層級與各自允許的異常現象，請參閱 [BEP-161](./161.md)。
...
最終一致性的模式與取捨，請參閱 [BEP-165](./165.md)。
...
- **兩階段提交（Two-Phase Commit, 2PC）：** 協調器先讓所有參與者進入準備狀態，再提交。提供類 ACID 的保證，但速度慢、複雜，且在協調器失敗時會阻塞。請參閱 [BEP-162](./162.md)。
- **Saga 模式：** 將分散式操作拆解為一系列本地交易，並為失敗提供補償動作。請參閱 [BEP-163](./163.md)。
...
在不理解預設值的情況下接受它，是一個潛在的 bug。PostgreSQL 的預設值（Read Committed）在並發負載下允許不可重複讀取（non-repeatable reads）和遺失更新（lost updates）。請根據你的工作負載評估每個層級允許的異常現象。請參閱 [BEP-161](./161.md)。
...
- [BEP-161: 隔離層級與其異常現象](./161.md) -- 深入探討各隔離層級及其允許的異常現象
- [BEP-162: 分散式交易與兩階段提交](./162.md) -- 跨服務協調交易
- [BEP-163: Saga 模式](./163.md) -- 長時間執行的分散式工作流程的補償交易
- [BEP-165: 最終一致性模式](./165.md) -- 何時以可用性換取 ACID 保證
