...
- **BEP-161** -- 交易隔離等級：隔離等級決定交易可以看到哪些資料；悲觀鎖在隔離之上增加了明確的寫入序列化。
- **BEP-241** -- 競爭條件：兩種並行策略都是消除共享資料上競爭條件的工具。
- **BEP-241** -- 鎖與死鎖：使用悲觀控制時適用的死鎖分析、鎖排序與偵測策略。
- **BEP-71** -- API 冪等性：樂觀衝突上的重試邏輯需要冪等操作，以避免重複套用變更。
