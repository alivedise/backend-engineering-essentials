...
**P7 — 信任身分 claims 前，必須完整驗證 ID token。** ID token 是一個 JWT，所有 JWT 驗證規則均適用（見 [BEE-1002](token-based-authentication.md)）：驗證簽名、`iss`、`aud`、`exp` 及 `nonce`（使用時）。不應以 access token 或 `/userinfo` 回應作為 ID token 驗證的替代方案。
...
使用 access token 呼叫 `GET /userinfo` 並不能向你的應用程式認證使用者——它只是從授權伺服器取得 claims，但沒有與當前 session 或使用者互動行為的綁定關係。應使用 OpenID Connect：驗證 ID token 的簽名與 `nonce` claim，以確認特定使用者確實在此特定 session 中完成了認證。（AuthN/AuthZ 的區別詳見 [BEE-1001](authentication-vs-authorization.md)。）
