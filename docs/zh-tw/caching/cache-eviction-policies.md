...
:::tip 淘汰 vs. 失效
基於 TTL 的過期是快取失效（參見 [BEE-9002](cache-invalidation-strategies.md)）的一種形式，而非淘汰。淘汰由記憶體壓力驅動；失效由資料過時性驅動。兩者各自獨立運作。
:::
...
- [BEE-9001](caching-fundamentals-and-cache-hierarchy.md) -- 快取基礎：快取模式（cache-aside、write-through、write-behind）以及何時加入快取層。
- [BEE-9002](cache-invalidation-strategies.md) -- 快取失效策略：TTL、事件驅動失效、write-through。失效由資料過時性驅動；淘汰由記憶體壓力驅動。兩者相互影響，但各自獨立。
- [BEE-9004](distributed-caching.md) -- 分散式快取：叢集與分片快取中淘汰策略的行為，以及一致性雜湊對每節點容量的影響。
