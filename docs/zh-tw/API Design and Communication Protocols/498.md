...
- [BEE-70](70.md) -- REST API 設計原則：OpenAPI 規範應強制執行的 REST 慣例（資源命名、狀態碼、HTTP 方法）
- [BEE-71](71.md) -- API 版本化策略：如何在 OpenAPI `info.version` 欄位和 servers 陣列中表達版本變更
- [BEE-75](75.md) -- API 錯誤處理和問題詳情：`components/responses` 中使用的 `application/problem+json` Schema
- [BEE-342](342.md) -- 契約測試：基於 Prism 和 openapi-generator 的客戶端提供 Pact 風格消費者驅動契約的替代方案
- [BEE-360](360.md) -- 持續整合原則：Spectral Linting 和 oasdiff 重大變更檢查屬於 CI 管線
