...
完整的 REST 架構限制與 HTTP 方法語意請參閱 [BEE-70](70.md)。
...
gRPC 依賴的 HTTP/2 基礎知識（包含多路復用與標頭壓縮）請參閱 [BEE-52](../Networking Fundamentals/52.md)。
...
- [BEE-52](../Networking Fundamentals/52.md) HTTP/2 與傳輸層基礎（gRPC 的底層依賴）
- [BEE-70](70.md) REST API 設計原則
- [BEE-76](76.md) Webhook 與回呼模式（非同步通訊）
