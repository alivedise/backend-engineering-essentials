...
:::tip 深入探討
關於 API 層級的冪等性設計模式與治理，請參閱 [ADP (API Design Principles)](https://alivedise.github.io/api-design-principle-beta/)。
:::
...
- **BEP-70** — REST API 設計原則：HTTP 方法語意與資源建模
- **BEP-164** — 訊息傳遞中的冪等性：將相同保證應用於非同步訊息佇列
- **BEP-261** — 重試策略：指數退避、抖動與客戶端重試預算
