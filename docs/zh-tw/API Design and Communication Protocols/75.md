...
在內部記錄完整錯誤（例外、堆疊追蹤、請求上下文），以相同的 `correlation_id` 作為索引鍵。當客戶端回報錯誤時，關聯 ID 是找到完整診斷軌跡所需的唯一資訊。結構化日誌慣例請參見 [BEP-321](../../Observability and Reliability/321.md)。
...
- [BEP-31](../../Data Management and Storage/31.md) 輸入驗證
- [BEP-321](../../Observability and Reliability/321.md) 結構化日誌
