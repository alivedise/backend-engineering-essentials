...
[BEE-597](597.md) 涵蓋了請求端的缺口：REST 從 HTTP 繼承基礎設施，GraphQL 必須自己重建。本文涵蓋回應端的對應部分。同樣的論點適用：單一的 `POST /graphql` 端點壓縮掉了 HTTP 為 REST 免費提供的三件事。
...
**REST 基線。** URL pattern 是天然的標籤。`http.route="/products/:id"` 是 W3C HTTP 語意慣例定義、每個 OpenTelemetry HTTP instrumentation 函式庫發出的標準 span 屬性。一個標籤同時涵蓋 metric、trace、log。每路由的延遲儀表板是預設值，不是專案。請見 [BEE-322](../Observability/322.md) 了解這一切建立其上的 W3C trace context 與 span 模型。
...
3. **Persisted query 不透明。** Persisted query GET（[BEE-596](596.md)）讓 URL 更沒用。`/graphql?id=hash` 是某個 query 的標籤，但前提是可觀測性 stack 把 hash 對應回 operation name。
...
**REST 基線。** 在 URL/方法層執行授權。Gateway 層級的 RBAC（[BEE-14](../Authentication and Authorization/14.md)）把 `(角色, URL pattern, 方法)` 對應到 allow/deny，通常不必呼叫應用程式碼。ABAC 政策（Open Policy Agent、AWS Verified Permissions）在 gateway 評估 `(subject, resource, action)`。資源所有權檢查在 handler 中執行。兩層協作：gateway 以角色把關，應用程式以所有權把關。請見 [BEE-10](../Authentication and Authorization/10.md) 了解認證 vs 授權的基線。
...
**REST 基線。** HTTP 狀態碼是失敗訊號的標準通道：4xx 給客戶端錯誤、5xx 給伺服器錯誤，搭配 [RFC 9457 Problem Details](https://www.rfc-editor.org/rfc/rfc9457.html)（[BEE-75](75.md)）以 `application/problem+json` media type 提供機器可讀的錯誤主體。狀態碼驅動每一個感知 HTTP 的工具：負載平衡器健康檢查、重試函式庫、監控儀表板、CDN 日誌。主體提供欄位層級的細節與 correlation ID。兩個通道（狀態碼給類別、主體給細節）就是 HTTP 基礎設施依賴的契約。
...
- [BEE-75](75.md) API 錯誤處理與 Problem Details — REST 標準處理；本文直接引用其反模式列舉
- [BEE-72](72.md) API 中的冪等性 — RFC 9110 動詞語意是 REST 狀態碼的基礎
- [BEE-597](597.md) GraphQL vs REST：請求端的 HTTP 取捨 — 姊妹文章（速率限制小節討論 429 + Retry-After）

**可觀測性叢集：**

- [BEE-322](../Observability/322.md) 分散式追蹤 — W3C trace context、span 模型、採樣
- [BEE-321](../Observability/321.md) 結構化日誌 — correlation ID、JSON log 格式
- [BEE-320](../Observability/320.md) 三大支柱：Logs、Metrics、Traces — 框架基礎

**授權叢集：**

- [BEE-10](../Authentication and Authorization/10.md) 認證 vs 授權 — 定義基線
- [BEE-14](../Authentication and Authorization/14.md) RBAC vs ABAC 存取控制模型 — REST 授權模式
- [BEE-12](../Authentication and Authorization/12.md) OAuth 2.0 與 OpenID Connect — token-based auth 脈絡
- [BEE-499](../Security Fundamentals/499.md) Broken Object Level Authorization (BOLA) — 相鄰的安全議題
