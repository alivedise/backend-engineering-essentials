...
- [BEE-508](508.md) -- 結構化輸出與受限解碼：在標記層面強制執行 Schema 合規性的解碼層機制（語法約束、函數呼叫）
- [BEE-509](509.md) -- RAG 流程架構：本文的文件解析與分塊直接饋入 RAG 流程的索引階段
- [BEE-527](527.md) -- LLM 批次處理模式：OpenAI 和 Anthropic 的批次 API 可直接應用於大量文件提取工作負載
- [BEE-524](524.md) -- 人工介入 AI 模式：本文的人工審查路由模式是 BEE-524 中信心度閘控人工審查模式的一個實例
