...
- [BEE-514](514.md) -- 微調與 PEFT 模式：LoRA 和 QLoRA 減少了可訓練參數的數量，大幅降低了 ZeRO 必須分片的梯度和優化器狀態佔用
- [BEE-563](563.md) -- LLM 推論量化：量化減少推論記憶體；訓練時 QLoRA（4 位元基礎權重 + BF16 LoRA）將兩種技術結合
- [BEE-568](568.md) -- LLM 推論的張量平行與流水線平行：三維平行將基於 ZeRO 的資料平行與 TP/PP 結合，用於最大規模的訓練
- [BEE-570](570.md) -- LLM 服務的自動擴縮與 GPU 叢集管理：訓練的 GPU 叢集管理原則與服務有顯著重疊
