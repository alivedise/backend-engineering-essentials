...
- [BEE-261](261.md) -- 重試策略與指數退避：LLM 特定退避擴展的通用重試模式
- [BEE-266](266.md) -- 速率限制與節流：為自己的 API 實施伺服器端速率限制（與客戶端供應商配額管理互補）
- [BEE-513](513.md) -- AI 成本最佳化與模型路由：路由至較便宜的模型是降低 Token 消耗的互補策略
- [BEE-526](526.md) -- LLM 快取策略：語意快取減少 Token 消耗，直接緩解 ITPM 壓力
- [BEE-527](527.md) -- LLM 批次處理模式：批次 API 繞過即時速率限制，是高量非互動工作負載的正確選擇
