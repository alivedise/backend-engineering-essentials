...
- [BEE-503](503.md) -- LLM API 整合模式：令牌管理、語義快取、串流和重試模式適用於代理循環中的個別 LLM 呼叫
- [BEE-499](499.md) -- 物件層級授權失效（BOLA）：存取物件 ID 的代理工具呼叫必須執行物件層級授權，與 REST 端點要求相同
- [BEE-462](462.md) -- 稽核日誌架構：每個代理工具呼叫必須記錄完整的上下文；稽核 Schema 應引用執行 ID、步驟編號和用戶身份
- [BEE-383](383.md) -- 向量搜尋與語義搜尋：外部代理記憶是作為向量存儲實現的；相同的檢索模式適用
- [BEE-482](482.md) -- 零信任安全架構：多代理系統必須將代理間通訊視為不受信任的；零信任原則在代理之間的適用程度與在服務之間相同
