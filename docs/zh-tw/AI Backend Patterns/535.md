...
- [BEE-508](508.md) -- 結構化輸出與受限解碼：保持生成程式碼可解析格式的語法約束和函數呼叫機制
- [BEE-520](520.md) -- LLM 工具使用與函數呼叫模式：程式碼生成代理使用工具呼叫來調用沙盒執行器、程式碼檢查器和測試運行器
- [BEE-506](506.md) -- 評估和測試 LLM 應用：SWE-bench 和 HumanEval 是黃金資料集評估模式的程式碼專屬實例
