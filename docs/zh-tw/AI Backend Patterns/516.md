...
- [BEE-509](509.md) -- RAG 管線架構：嵌入模型是 RAG 索引和查詢管線的核心元件；塊大小決策與模型的上下文視窗直接交互
- [BEE-383](../Search/383.md) -- 向量搜尋與語義搜尋：ANN 索引結構（HNSW、IVF）和近似搜尋的權衡，直接適用於基於嵌入的檢索
- [BEE-513](513.md) -- AI 成本優化與模型路由：嵌入 API 呼叫有其自身的每 Token 費用，在索引規模下會累積；快取和量化是費用槓桿
- [BEE-511](511.md) -- LLM 可觀測性與監控：嵌入延遲和快取命中率應作為獨立指標，與 RAG 系統中的生成延遲分開追蹤
