...
- [BEE-523](523.md) -- LLM 推理優化與自託管：用於高效服務量化模型的 vLLM、連續批處理和 KV 快取優化
- [BEE-514](514.md) -- 微調與 PEFT 模式：用於在不完全重訓的情況下將量化模型適應特定任務的 LoRA 和 QLoRA
- [BEE-513](513.md) -- AI 成本優化與模型路由：將請求路由至最小可用模型是模型壓縮的運營補充
