...
- [BEE-504](504.md) -- AI 代理架構模式：規劃與任務分解運作其中的代理架構
- [BEE-520](520.md) -- LLM 工具使用與函式呼叫模式：ReAct 行動呼叫工具的機制
- [BEE-537](537.md) -- AI 代理安全與可靠性模式：代理預算上限和熔斷器限制規劃迴圈的執行時間
- [BEE-529](529.md) -- AI 工作流程協調：用於確定性工作流程的 DAG 協調是 LLM 生成的動態計畫的補充
- [BEE-525](525.md) -- 思維鏈與延伸思考模式：思維鏈是每個子任務的推理；規劃是跨子任務的結構
