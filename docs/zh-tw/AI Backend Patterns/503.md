...
- [BEE-34](34.md) -- 工程師的密碼學基礎：嵌入模型生成稠密向量；理解餘弦相似度需要理解向量空間和距離度量
- [BEE-200](200.md) -- 快取基礎：語義快取是一個專門的快取失效問題——回應按語義接近度快取，而非精確鍵值
- [BEE-261](261.md) -- 重試策略與指數退避：LLM API 的重試模式與任何具有速率限制的外部服務相同
- [BEE-266](266.md) -- 速率限制與節流：LLM API 執行 RPM 和 TPM 速率限制；後端應實作客戶端速率限制以保持在配額內
- [BEE-383](383.md) -- 向量搜尋與語義搜尋：RAG 依賴向量搜尋來檢索相關區塊；嵌入和相似度搜尋概念是共享的
- [BEE-499](499.md) -- 物件層級授權失效（BOLA）：函式呼叫的繞過必須通過與 REST 端點相同的物件層級授權檢查來防止
