...
- [BEE-544](544.md) -- AI 紅隊測試與對抗性測試：對抗性測試是第 15 條（準確率、穩健性、資安）的要求；BEE-544 涵蓋饋入第 9 條風險管理系統的測試方法論
- [BEE-579](579.md) -- LLM 輸出浮水印與 AI 內容溯源：C2PA 內容憑證和浮水印處理第 50 條（AI 生成內容的透明度義務），補充了第 12 條的審計追蹤義務
- [BEE-510](510.md) -- LLM 安全性與提示注入：第 15 條要求資安；提示注入是基於 LLM 的高風險系統的主要資安攻擊面
- [BEE-322](322.md) -- 分散式追蹤：穿插在 AI 審計事件中的 request_id 應與分散式追蹤系統使用的追蹤 ID 相同，以實現 AI 決策與完整系統行為的關聯
