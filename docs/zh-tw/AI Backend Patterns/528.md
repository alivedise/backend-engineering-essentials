...
- [BEE-516](516.md) -- 嵌入模型與向量表示：涵蓋嵌入如何生成以及決定索引記憶體需求的維度選擇
- [BEE-509](509.md) -- RAG 管道架構：向量資料庫是每個 RAG 系統中的檢索層；集合設計和過濾策略直接影響 RAG 召回率
- [BEE-517](517.md) -- 檢索重排序與混合搜索：混合 BM25 + 向量搜索是 Weaviate 的原生功能；重排序階段位於向量資料庫檢索的下游
- [BEE-526](526.md) -- LLM 快取策略：語意快取使用向量索引來偵測近似重複的查詢；相同的 ANN 基礎設施服務於兩種目的
