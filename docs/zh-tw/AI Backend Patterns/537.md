...
- [BEE-504](504.md) -- AI Agent 架構模式：這些安全層所包裹的 Agent 迴圈設計
- [BEE-520](520.md) -- LLM 工具使用與函數呼叫模式：熔斷器與政策閘道攔截的工具調度機制
- [BEE-524](524.md) -- 人機協作 AI 模式：政策層觸發的核准閘道機制與基於信心度的路由
- [BEE-260](260.md) -- 熔斷器模式：此處套用於每個工具可靠性的通用熔斷器模式
- [BEE-461](461.md) -- N+1 查詢問題與批次載入：Agent 工具呼叫中的重試風暴是 N+1 查詢問題的 Agentic 類比
