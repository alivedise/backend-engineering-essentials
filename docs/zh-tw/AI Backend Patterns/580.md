...
- [BEE-509](509.md) -- RAG 流水線架構：Schema 連結使用與 RAG 相同的結構化上下文檢索模式；這些技術是可遷移的
- [BEE-510](510.md) -- LLM 安全性與提示注入：text-to-SQL 中的提示注入攻擊面（arXiv:2308.01990）是提示注入問題的直接實例
- [BEE-476](476.md) -- 資料庫行級安全：對於多租戶 text-to-SQL，資料庫層面的行級安全是 Schema 整理和唯讀權限的補充
- [BEE-508](508.md) -- 結構化輸出與約束解碼：強制模型輸出有效 SQL（約束解碼到 SQL 語法）可減少解析錯誤和幻覺識別符
