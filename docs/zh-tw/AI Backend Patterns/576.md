...
- [BEE-514](514.md) -- 微調與 PEFT 模式：LoRA 微調是模型合併最常見的輸入——mergekit 可以通過將 LoRA 適配器轉換為全秩任務向量後進行合併
- [BEE-573](573.md) -- RLHF 與對齊訓練基礎設施：取反任務向量（任務算術）可以減少有害行為；將安全微調模型與能力專業化模型合併是常見的對齊工作流程
- [BEE-562](562.md) -- Multi-LoRA 服務與適配器管理：模型合併是動態適配器切換的靜態替代方案——將適配器合併到一個檢查點消除了運行時開銷，代價是失去靈活性
- [BEE-572](572.md) -- 大型語言模型的分散式訓練基礎設施：理解權重空間幾何（損失盆地、任務向量）說明了為什麼分散式訓練必須保留模型出處以備將來合併
