...
- [BEE-567](567.md) -- 連續批次處理與迭代層級排程：分塊預填充部分緩解了分離架構所要解決的問題
- [BEE-568](568.md) -- LLM 推論的張量平行與流水線平行：在分離式部署中，預填充和解碼可使用不同的 TP/PP 配置
- [BEE-565](565.md) -- 前綴快取與 KV 快取複用：預填充叢集的前綴快取命中率可降低傳輸量，攤薄 KV 傳輸成本
- [BEE-523](523.md) -- LLM 推論優化與自主託管：更廣泛的服務基礎設施背景
