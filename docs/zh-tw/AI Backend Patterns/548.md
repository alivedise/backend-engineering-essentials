...
- [BEE-508](508.md) -- 結構化輸出與受約束解碼：本文受約束解碼層的底層 token 級語法強制機制
- [BEE-520](520.md) -- LLM 工具使用與函數調用模式：Anthropic tool use 是提供原生硬性 schema 強制執行的機制
- [BEE-545](545.md) -- LLM 幻覺偵測與事實接地：幻覺偵測與結構化提取互補；經驗證的 schema 不保證提取值的事實正確性
- [BEE-537](537.md) -- AI 智能體安全與可靠性模式：提取管道的重試預算和熔斷器遵循與智能體可靠性控制相同的模式
