...
- [BEE-30](../Security Fundamentals/30.md) -- 後端 OWASP Top 10：傳統 OWASP Top 10 對 LLM 應用程式仍然相關；注入和訪問控制損壞在兩個列表中都出現，但機制不同
- [BEE-32](../Security Fundamentals/32.md) -- 機密管理：通常放在環境變數中的機密往往被錯誤地放在 LLM 系統提示中——相同的機密管理原則適用
- [BEE-504](504.md) -- AI Agent 架構模式：過度代理（LLM06）在代理循環中特別危險；HITL 檢查點和工具授權在那裡有所涵蓋
- [BEE-509](509.md) -- RAG 管道架構：RAG 是主要的間接注入面；文件攝入管道安全和檢索信任層次直接適用
- [BEE-266](../Resilience and Reliability/266.md) -- 速率限制和節流：自動提示注入活動需要速率限制來偵測和控制
