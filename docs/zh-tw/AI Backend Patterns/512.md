...
- [BEE-509](509.md) -- RAG 管道架構：檢索 Chunk 是上下文預算的主要消耗者；Chunk 大小和重排序直接影響有多少文件能與對話歷史並存
- [BEE-503](503.md) -- LLM API 整合模式：語義快取和提示快取是依賴穩定提示前綴的成本降低技術——上下文結構影響快取命中率
- [BEE-511](511.md) -- LLM 可觀測性與監控：每次請求的 Token 計數和上下文利用率百分比是需要追蹤的關鍵指標；TTFT 隨上下文長度增加，應分開測量
- [BEE-504](504.md) -- AI Agent 架構模式：長時間運行的代理會累積工具呼叫歷史和中間結果，可能耗盡上下文視窗；記憶體管理模式直接適用
