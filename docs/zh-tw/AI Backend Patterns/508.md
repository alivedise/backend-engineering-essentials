...
- [BEE-503](503.md) -- LLM API 整合模式：Token 成本管理和語義快取直接適用於結構化輸出呼叫；Schema 傳輸 Token 計入那裡涵蓋的成本模型
- [BEE-504](504.md) -- AI Agent 架構模式：代理循環中的工具使用依賴結構化輸出進行參數傳遞；每次工具呼叫都是一個受限解碼問題
- [BEE-506](506.md) -- 評估與測試 LLM 應用程式：格式合規性是評估維度表中的維度之一；Schema 違規應作為回歸案例出現在黃金資料集中
- [BEE-143](../Modeling Data/143.md) -- 編碼與序列化格式：JSON Schema 作為序列化契約；嚴格與寬鬆 Schema 驗證之間的取捨在資料管道的背景下有所涵蓋
