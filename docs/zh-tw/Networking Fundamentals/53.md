...
這是 HTTP/2 透過 TLS 協商的方式（參見 [BEE-52](52.md)）。沒有 ALPN，就需要額外一次往返在連線建立後才能切換協定。
...
- [BEE-34](34.md) — 密碼學基礎：對稱加密、雜湊、數位簽章（TLS 使用的建構模塊）
- [BEE-50](50.md) — TCP/IP：TLS 在 TCP 之上運行；理解 TCP 握手可釐清總連線延遲（TCP SYN + TLS = 資料前 2-3 RTT）
- [BEE-52](52.md) — HTTP/2 與 HTTP/3：ALPN 在 TLS 上協商 HTTP/2；HTTP/3 使用內建 TLS 1.3 的 QUIC
- [BEE-54](54.md) — 負載均衡器：大規模環境的 TLS 終止策略、SNI 路由和憑證管理
