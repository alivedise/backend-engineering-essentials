...
這是 HTTP/2 透過 TLS 協商的方式（參見 BEP-52）。沒有 ALPN，就需要額外一次往返在連線建立後才能切換協定。
...
- **BEP-34** — 密碼學基礎：對稱加密、雜湊、數位簽章（TLS 使用的建構模塊）
- **BEP-50** — TCP/IP：TLS 在 TCP 之上運行；理解 TCP 握手可釐清總連線延遲（TCP SYN + TLS = 資料前 2-3 RTT）
- **BEP-52** — HTTP/2 與 HTTP/3：ALPN 在 TLS 上協商 HTTP/2；HTTP/3 使用內建 TLS 1.3 的 QUIC
- **BEP-54** — 負載均衡器：大規模環境的 TLS 終止策略、SNI 路由和憑證管理
