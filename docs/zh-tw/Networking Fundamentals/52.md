...
- [BEP-50 — TCP/IP and the Network Stack](./50.md)：TCP 握手、慢啟動，以及為何 HTTP/3 以 QUIC 取代 TCP。
- [BEP-53 — TLS/SSL Handshake](./53.md)：ALPN 擴充、TLS 1.3 與 QUIC 的整合、憑證協商。
- [BEP-54 — Load Balancers](./54.md)：HTTP/2 與 HTTP/3 的第 7 層終止，對上游的協定翻譯。
- [BEP-70 — REST API Design](../API%20Design/70.md)：協定版本如何影響 API 延遲與用戶端連線策略。
- [BEP-205 — HTTP Caching](../Caching/205.md)：Cache-Control 語意定義於 RFC 9110，在所有 HTTP 版本中完全相同。
