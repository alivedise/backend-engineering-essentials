...
**資料庫考量：** 若 v2 包含 Schema 變更，綠環境必須使用 v1 也能讀取的 Schema（先擴充再收縮模式，expand-before-contract）。請參閱 [BEE-6004](#) 了解資料庫遷移對齊方式。
...
| 相關 BEE | — | [BEE-16004 Feature Flags](#) |
...
- [BEE-6004：資料庫遷移](#) — 與部署策略對齊的 Schema 遷移模式；先擴充再收縮
- [BEE-15006：生產環境測試](#) — 冒煙測試、合成流量與可觀測性要求，作為部署進程的閘門
- [BEE-16001：持續整合](#) — 調用任何部署策略前必須通過的 CI Pipeline 閘門
- [BEE-16004：Feature Flags](#) — 將程式碼部署與功能發布解耦；與金絲雀和 A/B 模式互補
