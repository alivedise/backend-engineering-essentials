...
當與事件溯源結合使用（見 BEP-223）時，寫入模型不再直接儲存當前狀態。取而代之：
...
- [BEP-101](101.md) -- DDD 精要：聚合根產生的領域事件是 CQRS 投影的天然輸入來源
- [BEP-103](103.md) -- 六角形架構：命令處理器與查詢處理器是應用服務；六角形架構定義了它們所在的層次
- [BEP-163](163.md) -- Saga 模式：Saga 協調多步驟命令；CQRS 在每個步驟內部套用
- [BEP-223](223.md) -- 事件溯源：完整形式的 CQRS 使用事件儲存作為寫入端的權威資料來源
