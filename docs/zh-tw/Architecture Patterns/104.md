...
- **[BEE-100](100.md)（單體 vs 微服務 vs 模組化單體）** -- 絞殺者無花果是分解單體時的推薦遷移策略。[BEE-100](100.md) 定義架構目標；[BEE-104](104.md) 描述如何在不中斷流量的情況下抵達目標。
- **[BEE-101](101.md)（領域驅動設計）** -- DDD 的限界上下文直接對應絞殺者無花果的遷移單元。一次遷移一個限界上下文；上下文邊界提供外觀層分割流量的自然接縫。
- **[BEE-126](126.md)（資料庫遷移）** -- 資料遷移與服務遷移並行進行。[BEE-126](126.md) 深入涵蓋相關技術（雙寫、CDC、批量填充）。
...
- [BEE-100](100.md)：單體 vs 微服務 vs 模組化單體
- [BEE-101](101.md)：領域驅動設計精要
- [BEE-126](126.md)：資料庫遷移
