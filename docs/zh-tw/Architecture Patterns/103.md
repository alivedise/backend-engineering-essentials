...
- **DDD**（參見 [BEE-101](./101.md)）-- 六角形架構是 DDD 的天然結構補充。領域聚合、實體和值物件存在於核心；Repository 是被驅動端 Port。
- **CQRS**（參見 [BEE-101](./101.md)）-- 命令與查詢處理器是領域核心的使用案例；讀模型 Adapter 和寫 Adapter 是被驅動端 Adapters。
...
- [BEE-100](100.md)：架構模式概覽
- [BEE-101](101.md)：領域驅動設計
- [BEE-101](101.md)：CQRS
