...
**MUST（必須）將作業處理器設計為冪等的。** 至少一次投遞保證作業可能執行超過一次。發送付款扣款或發送電子郵件的作業必須無論執行多少次都產生相同的可觀察結果。冪等鍵、資料庫 upsert 和先檢查再行動的模式是標準技術（見 [BEE-473](../Distributed Systems/473.md)）。
...
- [BEE-305](../Performance and Scalability/305.md) -- 非同步處理與工作佇列：解耦非同步工作的範式與動機；本文涵蓋實作架構
- [BEE-164](../Transactions and Data Integrity/164.md) -- 冪等性與恰好一次語意：作業處理器必須是冪等的；本文深入介紹技術
- [BEE-473](../Distributed Systems/473.md) -- 冪等鍵實作模式：冪等作業處理器的具體實作
- [BEE-472](../Distributed Systems/472.md) -- Outbox 模式與事務性訊息：PostgreSQL 支援的佇列在單一事務中原生啟用 Outbox 模式
- [BEE-457](../Distributed Systems/457.md) -- 分散式作業排程：涵蓋位於單機作業佇列之上的叢集級排程器（Airflow、Quartz、Kubernetes CronJobs）
- [BEE-224](../Messaging and Event-Driven/224.md) -- 死信佇列與毒藥訊息：任務佇列中的死亡作業集合是訊息中介層 DLQ 的等效物
