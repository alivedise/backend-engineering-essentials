...
直接針對 SLO 燃燒率告警，是確保呼叫反映真實使用者影響同時最小化雜訊的最可靠方式。完整的 SLO 機制請參見 [BEE-14002](structured-logging.md)；以下是告警應用的摘要。
...
- [BEE-14001](three-pillars-logs-metrics-traces.md) — 三大支柱：日誌、指標、追蹤——告警建立其上的訊號基礎
- [BEE-14002](structured-logging.md) — SLO 與錯誤預算：定義可靠性目標並推導燃燒率告警
- [BEE-14006](health-checks-and-readiness-probes.md) — 健康檢查與就緒探針：存活性/就緒性作為告警的輸入
