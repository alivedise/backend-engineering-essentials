...
API 金鑰識別的是呼叫端應用程式，不識別使用者、不承載同意授權範圍（consent scope），也不支援委派。任何代表使用者執行的操作（存取使用者資料、發起使用者可見的交易），都應在 API 金鑰之外額外使用 OAuth 2.0 短期 access token，或以 OAuth client credentials grant 取代 API 金鑰進行機器對機器的流程。（參見 [BEE-11](11.md)）
