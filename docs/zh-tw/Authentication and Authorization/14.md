...
授權不只是一個有效的 token。確認身分之後（BEP-10），系統必須判斷已驗證的 principal 是否有權對目標資源執行所請求的操作。有兩個基礎模型決定這個判斷如何被編碼：
...
- [BEP-10: Authentication vs Authorization](/zh-tw/Authentication%20and%20Authorization/10) — 所有權限模型的前置管線
- [BEP-12: OAuth 2.0 and OpenID Connect](/zh-tw/Authentication%20and%20Authorization/12) — scope 如何在協定層面對應粗粒度 RBAC
- [BEP-101: API Gateway Patterns](/zh-tw/API%20Design%20and%20Contracts/101) — gateway-first 架構中的執行點
