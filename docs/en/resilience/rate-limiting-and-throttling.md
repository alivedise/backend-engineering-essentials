...
From the client side (and for services calling other services), receiving a 429 is not an error in the traditional sense — it is a signal to back off. See [BEE-12002](retry-strategies-and-exponential-backoff.md) for full retry-with-backoff guidance. Key points:
...
- [BEE-1006](../auth/api-key-management.md) (API Keys) — API keys are the primary identifier for per-client rate limiting; key rotation and scoping affect limit enforcement
- [BEE-4006](../api-design/api-error-handling-and-problem-details.md) (Error Handling) — 429 is an application-level error and must be handled explicitly, not treated as an unexpected exception
- [BEE-12002](retry-strategies-and-exponential-backoff.md) (Retry Strategies and Exponential Backoff) — retry behavior after receiving 429; always includes jitter and respects `Retry-After`
- [BEE-12005](graceful-degradation.md) (Graceful Degradation and Load Shedding) — rate limiting as one layer of load shedding; [BEE-12005](graceful-degradation.md) covers application-level shedding strategies
