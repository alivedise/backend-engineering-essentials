...
Log the full error (exception, stack trace, request context) internally, keyed to the same `correlation_id`. When a client reports an error, the correlation ID is the only piece of information needed to find the full diagnostic trail. See [BEE-321](321.md) for structured logging conventions.
...
- [BEE-31](31.md) Input Validation
- [BEE-321](321.md) Structured Logging
