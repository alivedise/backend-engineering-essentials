...
- [BEE-70](70.md) -- REST API Design Principles: REST conventions (resource naming, status codes, HTTP methods) that the OpenAPI spec should enforce
- [BEE-71](71.md) -- API Versioning Strategies: how to express version changes in the OpenAPI `info.version` field and servers array
- [BEE-75](75.md) -- API Error Handling and Problem Details: the `application/problem+json` schema used in `components/responses`
- [BEE-342](342.md) -- Contract Testing: Prism and openapi-generator–based clients provide an alternative to Pact-style consumer-driven contracts
- [BEE-360](360.md) -- Continuous Integration Principles: Spectral linting and oasdiff breaking-change checks belong in the CI pipeline
