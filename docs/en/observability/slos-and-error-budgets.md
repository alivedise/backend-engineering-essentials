...
Using two windows prevents both false positives (short spike) and missed incidents (gradual degradation). See [BEE-14004](alerting-philosophy.md) for implementation details.
...
- [BEE-12006](../resilience/chaos-engineering-principles.md) — Chaos Engineering: SLOs define the steady-state hypothesis for chaos experiments. Do not run a chaos experiment if the error budget is already exhausted.
- [BEE-14001](three-pillars-logs-metrics-traces.md) — Metrics Instrumentation: the raw counters and histograms that SLIs are computed from.
- [BEE-14004](alerting-philosophy.md) — Alerting: burn-rate alert implementation with Prometheus and Alertmanager.
