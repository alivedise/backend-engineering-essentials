...
**Choosing the right isolation level is an application design decision**, not a database default to accept blindly. See [BEE-8002](isolation-levels-and-their-anomalies.md) for a full treatment of isolation levels and the anomalies each one permits.
...
See [BEE-8006](eventual-consistency-patterns.md) for patterns and trade-offs in eventual consistency.
...
- **Two-Phase Commit (2PC):** A coordinator prepares all participants, then commits. Provides ACID-like guarantees but is slow, complex, and blocks on coordinator failure. See [BEE-8003](distributed-transactions-and-two-phase-commit.md).
- **Saga pattern:** Break the distributed operation into a sequence of local transactions with compensating actions for failure. See [BEE-8004](saga-pattern.md).
...
Accepting the default isolation level without understanding it is a latent bug. The PostgreSQL default (Read Committed) allows non-repeatable reads and lost updates under concurrent load. Evaluate your workload against the anomalies each level permits. See [BEE-8002](isolation-levels-and-their-anomalies.md).
