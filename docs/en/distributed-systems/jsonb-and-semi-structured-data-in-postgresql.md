...
- [BEE-6001](../data-storage/sql-vs-nosql-tradeoffs.md) -- SQL vs NoSQL Tradeoffs: JSONB closes much of the document-store gap without leaving the relational model; understand when a dedicated document database is still the right choice
- [BEE-6002](../data-storage/indexing-deep-dive.md) -- Indexing Deep Dive: GIN is one of PostgreSQL's specialized index types; understand how it differs from B-tree and when each is appropriate
- [BEE-18002](../multi-tenancy/tenant-isolation-strategies.md) -- Tenant Isolation Strategies: JSONB `metadata` columns are commonly used for tenant-specific attributes; generated columns and functional indexes make tenant_id queryable without schema changes
