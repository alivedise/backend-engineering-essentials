...
- [BEE-2005](../security-fundamentals/cryptographic-basics-for-engineers.md) -- Cryptographic Basics for Engineers: presigned URLs are HMAC-SHA256 signatures over a canonical request string; the bearer-token security model means URL confidentiality is equivalent to key confidentiality
- [BEE-13005](../performance-scalability/content-delivery-and-edge-computing.md) -- Content Delivery and Edge Computing: CDN-signed URLs extend presigned URL patterns to the edge; media streaming uses CDN-signed URLs rather than origin presigned URLs to benefit from geographic caching
- [BEE-4003](../api-design/api-idempotency.md) -- Idempotency in APIs: the upload initiation endpoint SHOULD be idempotent; using a client-supplied idempotency key prevents duplicate pending upload records if the client retries the request
