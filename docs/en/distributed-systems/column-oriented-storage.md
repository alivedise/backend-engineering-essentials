...
- [BEE-6005](../data-storage/storage-engines.md) -- Storage Engines: covers LSM Trees and B-Trees, both of which are row-oriented storage engines; column-oriented storage is a third model optimized for analytical read patterns
- [BEE-7004](../data-modeling/encoding-and-serialization-formats.md) -- Encoding and Serialization Formats: covers Avro, Protocol Buffers, and Thrift; Parquet and Arrow are the columnar equivalents for analytical workloads
- [BEE-6002](../data-storage/indexing-deep-dive.md) -- Indexing Deep Dive: column stores use statistics-based skipping (min/max per row group) rather than traditional B-tree indexes; understanding both helps choose the right tool
- [BEE-13004](../performance-scalability/profiling-and-bottleneck-identification.md) -- Profiling and Bottleneck Identification: I/O read amplification is the primary bottleneck that columnar storage eliminates; use profiling to confirm before migrating
