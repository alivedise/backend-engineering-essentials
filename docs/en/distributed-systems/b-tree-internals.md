...
- [BEE-6002](../data-storage/indexing-deep-dive.md) -- Indexing Deep Dive: index selection strategy (when to use composite indexes, partial indexes, covering indexes) is the application-level view of B-tree internals; understanding page splits and fill factor explains why index choices have the performance characteristics they do
- [BEE-6005](../data-storage/storage-engines.md) -- Storage Engines: B-trees are the dominant structure in disk-based storage engines (InnoDB, PostgreSQL heap+nbtree); storage engine architecture determines how B-tree pages are cached, written, and recovered
- [BEE-19024](log-structured-merge-trees.md) -- Log-Structured Merge Trees: LSM-trees are the primary alternative to B-trees for write-heavy workloads; the write-amplification vs read-amplification tradeoff between them determines which structure is appropriate for a given workload
- [BEE-19026](mvcc-multi-version-concurrency-control.md) -- MVCC: Multi-Version Concurrency Control: MVCC version churn creates dead index entries that accumulate in B-tree leaf pages; PostgreSQL's bottom-up deletion and deduplication optimizations were introduced specifically to manage this interaction
