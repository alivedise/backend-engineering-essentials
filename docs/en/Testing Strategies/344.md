...
# [BEP-344] Test Doubles: Mocks, Stubs, Fakes
...
Per BEP-103 (hexagonal architecture), **ports are natural mock boundaries**. Mock the port, not the objects behind it.
...
- [BEP-103 — Hexagonal Architecture](../Architecture/103.md): Ports are natural mock boundaries. Mock the port interface, not the objects behind it.
- [BEP-340 — Test Pyramid](./340.md): Unit tests with doubles form the base; the pyramid determines how many of each type to write.
- [BEP-341 — Integration Testing](./341.md): When NOT to mock — cases where real infrastructure is required for meaningful test coverage.
