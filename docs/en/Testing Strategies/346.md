...
- [BEE-340](340.md) -- The Testing Pyramid: where PBT fits — it augments unit tests at the base; it is not a replacement for integration or contract tests
- [BEE-341](341.md) -- Integration Testing for Backend Services: PBT can drive integration tests by generating realistic request sequences for stateful API testing
- [BEE-342](342.md) -- Contract Testing: both PBT and contract testing validate interface invariants; contract tests fix the schema, PBT generates inputs within it
- [BEE-344](344.md) -- Test Doubles: Mocks, Stubs, Fakes: in stateful PBT, the model plays the role of an in-memory fake for the real system
