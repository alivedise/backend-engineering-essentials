...
Unit tests exercise a single function, class, or module in isolation. External dependencies — databases, network calls, file systems — are replaced with test doubles (see BEE-344).
...
- **BEE-341** (Integration Testing for Backend Services) — detailed guidance on writing integration tests with real databases and test containers
- **BEE-342** (Contract Testing) — validate cross-service contracts without running full E2E suites
- **BEE-343** (Load Testing and Benchmarking) — performance validation outside the functional test pyramid
- **BEE-344** (Test Doubles: Mocks, Stubs, Fakes) — how to isolate units for the base of the pyramid without compromising test value
