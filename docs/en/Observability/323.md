...
Alerting directly on an SLO burn rate is the most reliable way to ensure pages reflect real user impact while minimizing noise. See BEE-321 for full SLO mechanics; the alerting application is summarized here.
...
- **BEE-320** — The three pillars: logs, metrics, traces — the signal foundation alerting is built on
- **BEE-321** — SLOs and error budgets: defining reliability targets and deriving burn rate alerts
- **BEE-325** — Health checks and readiness probes: liveness/readiness as inputs to alerting
