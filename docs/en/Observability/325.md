...
# [BEP-325] Health Checks and Readiness Probes
...
Health checks and circuit breakers (see BEP-260) share information about dependency health, but serve different consumers.
...
When Kubernetes sends `SIGTERM`, the pod enters a termination sequence. The readiness probe should immediately begin failing at this point, so the load balancer stops routing new requests to the pod before it closes connections. See BEP-261 for the full graceful degradation sequence.
...
Load balancers outside Kubernetes — ALB, NGINX, HAProxy — also require health check configuration. See BEP-51 for load balancer health check specifics.
...
- **BEP-51** — Load Balancer Health Checks: configuring health check intervals and thresholds at the load balancer level.
- **BEP-260** — Circuit Breaker: how open circuits should influence readiness state.
- **BEP-261** — Graceful Degradation: coordinating SIGTERM handling with readiness probe state during shutdown.
