...
When Kubernetes sends `SIGTERM`, the pod enters a termination sequence. The readiness probe should immediately begin failing at this point, so the load balancer stops routing new requests to the pod before it closes connections. See [BEE-261](261.md) for the full graceful degradation sequence.
...
Load balancers outside Kubernetes — ALB, NGINX, HAProxy — also require health check configuration. See [BEE-51](51.md) for load balancer health check specifics.
...
- [BEE-51](51.md) — Load Balancer Health Checks: configuring health check intervals and thresholds at the load balancer level.
- [BEE-260](260.md) — Circuit Breaker: how open circuits should influence readiness state.
- [BEE-261](261.md) — Graceful Degradation: coordinating SIGTERM handling with readiness probe state during shutdown.
