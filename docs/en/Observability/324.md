...
Using two windows prevents both false positives (short spike) and missed incidents (gradual degradation). See BEE-323 for implementation details.
...
- **BEE-265** — Chaos Engineering: SLOs define the steady-state hypothesis for chaos experiments. Do not run a chaos experiment if the error budget is already exhausted.
- **BEE-320** — Metrics Instrumentation: the raw counters and histograms that SLIs are computed from.
- **BEE-323** — Alerting: burn-rate alert implementation with Prometheus and Alertmanager.
