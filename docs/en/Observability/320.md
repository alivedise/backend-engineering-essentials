...
# [BEP-320] The Three Pillars: Logs, Metrics, Traces
...
Unstructured log lines cannot be searched by field or aggregated. When an incident occurs, you cannot filter `level=error AND service=order-service AND trace_id=X`. You are reduced to grepping free text at 2 AM. Always log in JSON or a structured format. See BEP-321.
...
- **BEP-321** — Structured logging: schema, log levels, and what not to log
- **BEP-322** — Distributed tracing: spans, sampling strategies, and context propagation
- **BEP-323** — Alerting philosophy: what to alert on and how to write good runbooks
- **BEP-324** — SLOs and error budgets: using metrics to define and track reliability targets
