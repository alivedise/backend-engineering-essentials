...
- [BEE-14001](../observability/three-pillars-logs-metrics-traces.md) -- The Three Pillars: Logs, Metrics, Traces: LLM observability extends the three-pillar model with a fourth dimension — quality — that requires separate instrumentation
- [BEE-30004](evaluating-and-testing-llm-applications.md) -- Evaluating and Testing LLM Applications: RAGAS metrics and LLM-as-judge scoring used in offline evaluation apply equally to the online evaluation pipeline described here
- [BEE-14005](../observability/slos-and-error-budgets.md) -- SLOs and Error Budgets: quality metrics (faithfulness > 0.90, hallucination rate < 2%) can be formalized as SLOs; error budgets determine how much quality degradation is acceptable before rollback
- [BEE-30001](llm-api-integration-patterns.md) -- LLM API Integration Patterns: semantic caching reduces both cost and cache miss rate — the cache hit rate metric surfaced by observability informs cache tuning decisions
- [BEE-16002](../cicd-devops/deployment-strategies.md) -- Deployment Strategies: canary prompt rollouts follow the same traffic-splitting and rollback mechanics as software canary deployments
