...
- [BEE-30014](embedding-models-and-vector-representations.md) -- Embedding Models and Vector Representations: covers how embeddings are generated and the dimensionality choices that determine index memory requirements
- [BEE-30007](rag-pipeline-architecture.md) -- RAG Pipeline Architecture: the vector database is the retrieval layer in every RAG system; collection design and filtering strategy directly affect RAG recall
- [BEE-30015](retrieval-reranking-and-hybrid-search.md) -- Retrieval Reranking and Hybrid Search: hybrid BM25 + vector search is a native feature of Weaviate; the reranking stage sits downstream of the vector database retrieval
- [BEE-30024](llm-caching-strategies.md) -- LLM Caching Strategies: semantic caching uses a vector index to detect near-duplicate queries; the same ANN infrastructure serves both purposes
