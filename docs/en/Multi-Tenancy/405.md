...
- [BEE-400](400.md) -- Multi-Tenancy Models: silo/pool/bridge — the isolation model determines whether telemetry backends can be shared or must be per-tenant
- [BEE-401](401.md) -- Tenant Isolation Strategies: data plane isolation (RLS, namespaces) and control plane isolation (this article's focus: telemetry)
- [BEE-402](402.md) -- Tenant-Aware Rate Limiting: rate limit decisions should feed back into observability as quota-consumption metrics labeled by tenant
- [BEE-320](320.md) -- The Three Pillars: Logs, Metrics, Traces: foundational observability concepts; this article extends them with tenant-aware dimensions
- [BEE-324](324.md) -- SLOs and Error Budgets: per-tenant SLOs require per-tenant error rate metrics as their input signal
