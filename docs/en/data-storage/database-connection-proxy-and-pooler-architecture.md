...
- [BEE-6006](connection-pooling-and-query-optimization.md) -- Connection Pooling and Query Optimization: covers connection pooling from the application perspective and query-level optimization
- [BEE-13003](../performance-scalability/connection-pooling-and-resource-management.md) -- Connection Pooling and Resource Management: covers pool sizing theory and resource management at the application layer
- [BEE-18007](../multi-tenancy/database-row-level-security.md) -- Database Row-Level Security: `SET LOCAL app.current_tenant_id` must be used (not bare `SET`) when RLS relies on session variables in a PgBouncer transaction pooling environment
- [BEE-19034](../distributed-systems/graceful-shutdown-and-connection-draining.md) -- Graceful Shutdown and Connection Draining: the pooler layer must drain connections gracefully during deployment to avoid in-flight transaction loss
