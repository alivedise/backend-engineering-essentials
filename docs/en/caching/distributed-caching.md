...
- **Availability**: A single cache node is a single point of failure. When it crashes, every read goes to the database simultaneously -- a thundering herd (see [BEE-20](20.md)4).
...
A common pattern in large deployments is **cluster per region**: each data center runs a full cache cluster, and replication between regions is handled at the application or database layer rather than at the cache layer (see [BEE-8006](../transactions/eventual-consistency-patterns.md) on eventual consistency). This avoids cross-region cache writes on every request.
...
- [BEE-6004](../data-storage/partitioning-and-sharding.md) -- Database Sharding: consistent hashing applies identically to partitioning databases across nodes.
- [BEE-9001](caching-fundamentals-and-cache-hierarchy.md) -- Caching Fundamentals: cache-aside, write-through, write-behind patterns and the fundamentals this article builds on.
- [BEE-9005](cache-stampede-and-thundering-herd.md) -- Cache Stampede and Thundering Herd: what happens when distributed cache nodes fail and keys miss simultaneously.
- [BEE-8006](../transactions/eventual-consistency-patterns.md) -- Eventual Consistency: cross-region cache coherence and the consistency tradeoffs in distributed systems.
