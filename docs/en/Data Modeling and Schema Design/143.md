...
**When to use Protobuf:** Internal gRPC services, performance-critical internal APIs, any synchronous RPC path where both sides control the schema. See [BEE-74](74.md).
...
**When to use Avro:** Kafka topics, data pipeline events, batch data formats (Hadoop, Spark), any high-volume asynchronous stream where schema evolution and storage efficiency are both critical. See [BEE-220](220.md).
...
See [BEE-142](142.md) for the full rules of safe schema evolution regardless of format.
...
- [BEE-74](74.md) — gRPC and Protocol Buffers: Protobuf in synchronous RPC contexts
- [BEE-142](142.md) — Schema Evolution and Backward Compatibility: safe change rules for all formats
- [BEE-220](220.md) — Messaging and Event Streaming: Avro as the standard for Kafka topics
