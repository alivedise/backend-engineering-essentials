...
See BEE-71 (API Versioning) for the broader versioning strategy when a clean break is unavoidable.
...
Best practices:
- All schema changes must be applied via migrations (see BEE-126).
...
- **BEE-71** — API Versioning: when schema evolution is insufficient and a version bump is required
- **BEE-126** — Database Migrations: applying schema changes to persistent storage safely
- **BEE-143** — Encoding Formats: choosing between Protobuf, Avro, JSON, and MessagePack
