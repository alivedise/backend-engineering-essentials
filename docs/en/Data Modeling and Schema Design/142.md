...
# [BEP-142] Schema Evolution and Backward Compatibility
...
See BEP-71 (API Versioning) for the broader versioning strategy when a clean break is unavoidable.
...
Best practices:
- All schema changes must be applied via migrations (see BEP-126).
...
- **BEP-71** — API Versioning: when schema evolution is insufficient and a version bump is required
- **BEP-126** — Database Migrations: applying schema changes to persistent storage safely
- **BEP-143** — Encoding Formats: choosing between Protobuf, Avro, JSON, and MessagePack
