...
Event sourcing is suited to domains where history is the primary artifact (financial ledgers, compliance systems, CQRS-based microservices). It adds significant complexity — full replay time, snapshot management, projection consistency — and should not be adopted lightly. See BEE-223.
...
Without explicit indexes on the time column, range scans over large partitions will be sequential. See BEE-121.
...
Temporal queries are almost always range queries: `WHERE recorded_at BETWEEN '2025-01-01' AND '2025-01-31'`. Without an index on the time column, every query is a sequential scan. For partitioned tables, ensure the index is defined on each partition (or defined on the parent, which PostgreSQL propagates automatically). See BEE-121.

---

## Related BEPs

- **BEE-121** — Indexing Strategy: indexing for time range queries and covering indexes
- **BEE-123** — Partitioning by Time: detailed guidance on partition granularity and management
- **BEE-223** — Event Sourcing: full event sourcing architecture and projection patterns
