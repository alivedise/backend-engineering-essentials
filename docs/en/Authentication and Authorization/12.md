...
# [BEP-12] OAuth 2.0 and OpenID Connect
...
**P7 — Validate the ID token fully before trusting identity claims.** The ID token is a JWT. All JWT validation rules apply (see BEP-11): verify signature, `iss`, `aud`, `exp`, and `nonce` (when used). Do not use the access token or the `/userinfo` response as a substitute for ID token validation.
...
Calling `GET /userinfo` with an access token does not authenticate the user to your application — it fetches claims from the authorization server, but provides no binding to the current session or user interaction. Use OpenID Connect: verify the ID token's signature and `nonce` claim to establish that a specific user authenticated in this specific session. (See BEP-10 for the AuthN/AuthZ distinction.)
...
- [BEP-10: Authentication vs Authorization](/en/Authentication%20and%20Authorization/10) — the conceptual boundary that OAuth 2.0 and OIDC operate across
- [BEP-11: Token-Based Authentication](/en/Authentication%20and%20Authorization/11) — JWT structure, validation, and the token lifecycle
- [BEP-33: Third-Party API Integration Security](/en/Security%20Fundamentals/33) — OAuth in the context of external service integration
