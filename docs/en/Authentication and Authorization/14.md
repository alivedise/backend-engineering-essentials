...
# [BEP-14] RBAC vs ABAC Access Control Models
...
Authorization is more than a valid token. Once identity is established (BEP-10), the system must decide whether the verified principal is permitted to perform the requested action on the requested resource. Two foundational models govern how that decision is encoded:
...
- [BEP-10: Authentication vs Authorization](/en/Authentication%20and%20Authorization/10) — the pipeline that precedes any permission model
- [BEP-12: OAuth 2.0 and OpenID Connect](/en/Authentication%20and%20Authorization/12) — how scopes relate to coarse-grained RBAC at the protocol level
- [BEP-101: API Gateway Patterns](/en/API%20Design%20and%20Contracts/101) — enforcement points in a gateway-first architecture
