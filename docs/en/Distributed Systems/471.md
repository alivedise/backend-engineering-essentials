...
- [BEE-163](../Transactions and Consistency/163.md) -- Saga Pattern: Sagas are the distributed transaction pattern that requires either choreography or orchestration to coordinate compensating transactions across services
- [BEE-221](../Messaging and Event-Driven/221.md) -- Publish-Subscribe Pattern: choreography is built on pub/sub; understanding fan-out, consumer groups, and event ordering is prerequisite for choreography-based workflows
- [BEE-102](../Architecture Patterns/102.md) -- CQRS: orchestrators are a natural fit for the command side of CQRS; the orchestrator handles commands and produces events that update read models
- [BEE-222](../Messaging and Event-Driven/222.md) -- Delivery Guarantees: in choreography, event delivery guarantees (at-least-once, exactly-once) determine whether event handlers must be idempotent — they must
