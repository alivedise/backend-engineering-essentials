...
- [BEE-76](../API Design and Communication Protocols/76.md) -- Webhooks and Callback Patterns: covers webhook architectural design (retry strategies, delivery ordering, fan-out); this article covers the security layer that every webhook receiver must implement
- [BEE-32](../Security Fundamentals/32.md) -- Secrets Management: the shared HMAC secret must be stored and rotated using the same practices as any symmetric key — environment variables, secrets managers, never in source code
- [BEE-34](../Security Fundamentals/34.md) -- Cryptographic Basics for Engineers: HMAC-SHA256 is a keyed hash function; understanding hash properties (collision resistance, one-wayness) explains why signature verification provides the guarantees it does
