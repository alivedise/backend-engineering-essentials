...
- [BEE-125](../Data Storage and Database Fundamentals/125.md) -- Connection Pooling and Query Optimization: N+1 queries multiply connection usage; a connection pool exhausted by N+1 in one endpoint degrades all other endpoints
- [BEE-121](../Data Storage and Database Fundamentals/121.md) -- Indexing Deep Dive: N+1 queries each hit the index individually; even a fast indexed lookup becomes slow when repeated N times; batch loading amortizes the index traversal cost
- [BEE-74](../API Design and Communication Protocols/74.md) -- GraphQL vs REST vs gRPC: GraphQL's per-field resolver model is the structural cause of N+1 in GraphQL; DataLoader is the canonical mitigation
- [BEE-200](../Caching/200.md) -- Caching Fundamentals: DataLoader's per-request cache is a minimal, correct cache for deduplication within a request; it complements but does not replace a persistent cache layer
