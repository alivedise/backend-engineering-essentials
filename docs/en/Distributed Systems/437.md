...
- [BEE-430](430.md) -- Write-Ahead Logging: CDC is fundamentally a consumer of the WAL — PostgreSQL's logical decoding translates the binary WAL into row-level change events; understanding WAL structure (LSN, checkpoints) is essential for reasoning about CDC lag and offset management
- [BEE-223](../Messaging%20%26%20Event-Driven/223.md) -- Event Sourcing: CDC and event sourcing both treat a log of changes as the source of truth; CDC derives events from an existing database's replication log without changing how the application writes; event sourcing structures the application to write events first
- [BEE-162](../Transactions/162.md) -- Distributed Transactions and Two-Phase Commit: the outbox pattern is an alternative to 2PC for atomic database-and-message-bus updates; it achieves atomicity through a single-database transaction rather than a distributed coordinator
- [BEE-164](../Transactions/164.md) -- Idempotency and Exactly-Once Semantics: CDC delivers at-least-once; combining Kafka transactional producers with idempotent consumers achieves exactly-once end-to-end; the outbox row's UUID primary key serves as a natural idempotency key
