...
- [BEE-10](../Authentication and Authorization/10.md) -- Authentication vs Authorization: covers the foundational distinction; service-to-service auth applies the same principles but with automated credential management instead of interactive user login
- [BEE-12](../Authentication and Authorization/12.md) -- Token-Based Authentication: JWT structure and validation apply identically to service tokens; service tokens add `aud` scoping and automation requirements
- [BEE-32](../Security Fundamentals/32.md) -- Secrets Management: service credentials (private keys, signing secrets) must be stored and rotated through a secrets manager, not hardcoded or stored in environment variables baked into images
- [BEE-53](../Networking Fundamentals/53.md) -- TLS/SSL Handshake: mTLS extends the standard TLS handshake with client certificate presentation; understanding the base TLS handshake is prerequisite to understanding mTLS
- [BEE-462](462.md) -- Audit Logging Architecture: service identity must appear in audit log entries to enable cross-service forensic investigation
