...
- [BEE-74](../API Design/74.md) -- GraphQL vs REST vs gRPC: covers when to choose gRPC over REST; error handling is one of gRPC's differentiators
- [BEE-261](../Resilience/261.md) -- Retry Strategies and Exponential Backoff: the general retry theory; gRPC service config implements this at the channel layer
- [BEE-262](../Resilience/262.md) -- Timeouts and Deadlines: deadline propagation interacts with `DEADLINE_EXCEEDED`; understand the relationship between deadlines and status codes
- [BEE-465](../Distributed Systems/465.md) -- gRPC Streaming Patterns: streaming RPCs use the same status code model; a stream terminates with a single trailing status
