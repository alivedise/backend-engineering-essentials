...
- [BEE-143](../Data Modeling and Schema Design/143.md) -- Encoding and Serialization Formats: covers Avro, Protocol Buffers, Thrift, and MessagePack at a comparison level; this article provides the depth needed to safely evolve protobuf schemas in production
- [BEE-142](../Data Modeling and Schema Design/142.md) -- Schema Evolution and Backward Compatibility: the general principles (backward compatibility, forward compatibility, required vs optional) apply equally to protobuf, Avro, and SQL schemas
- [BEE-465](465.md) -- gRPC Streaming Patterns: gRPC uses protobuf as its IDL and wire format; schema evolution governs which proto changes require a new gRPC major version vs which can be deployed with rolling upgrades
- [BEE-126](../Data Storage and Database Fundamentals/126.md) -- Database Migrations: protobuf schema evolution and SQL schema migration share the same constraint — changes must be deployable without a coordinated cutover of all producers and consumers simultaneously
