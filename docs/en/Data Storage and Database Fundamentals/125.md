...
Columns that appear in `WHERE`, `JOIN ON`, and `ORDER BY` clauses are candidates for indexing. See **BEE-121** for indexing strategy.
...
- **BEE-121** — Database Indexing Strategies: which columns to index and how
- **BEE-301** — Resource Management: connection lifecycle and cleanup patterns
- **BEE-303** — Profiling and Observability: detecting slow queries and N+1 in production
