...
Consistent hashing is used by Cassandra, DynamoDB, and distributed caching systems (see BEE-203). Vitess uses a variant: it defines explicit key ranges (hex-encoded binary strings) per shard and adjusts ranges during resharding operations.
...
- [BEE-203: Distributed Caching](../../Caching%20and%20Performance/203.md) — consistent hashing is used in cache cluster routing with the same principles
