...
# [BEP-123] Partitioning and Sharding
...
Consistent hashing is used by Cassandra, DynamoDB, and distributed caching systems (see BEP-203). Vitess uses a variant: it defines explicit key ranges (hex-encoded binary strings) per shard and adjusts ranges during resharding operations.
...
For celebrity rows: the partition key approach cannot fully solve this — a single key is still a single partition. Mitigations include application-level caching (BEP-203), read replicas on the hot partition, or splitting the celebrity record into multiple virtual records with different keys (requires application logic to merge on read).
...
Do not shard prematurely. A single well-tuned PostgreSQL or MySQL instance can handle datasets in the hundreds of gigabytes and tens of thousands of QPS with proper indexing (BEP-121). Sharding adds significant operational overhead. Reach for it when you have a concrete capacity problem that cannot be solved by vertical scaling, read replicas, or caching.
...
- [BEP-120: SQL vs NoSQL](./120.md) — storage engine choice often determines sharding model options
- [BEP-121: Indexing Deep Dive](./121.md) — secondary index behavior in sharded systems
- [BEP-121: Replication Strategies](./121.md) — shards are typically replicated; partitioning and replication compose
- [BEP-203: Distributed Caching](../../Caching%20and%20Performance/203.md) — consistent hashing is used in cache cluster routing with the same principles
