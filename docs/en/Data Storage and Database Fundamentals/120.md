...
# [BEP-120] SQL vs NoSQL Tradeoffs
...
:::tip Deep Dive
For database-level implementation details on relational vs non-relational design, see [DDP-14: Relational vs Non-Relational](https://alivedise.github.io/database-design-principles/12).
:::
...
- Vertical scaling has limits; horizontal sharding is possible but adds complexity (see BEP-123).
...
SQL databases do scale — PostgreSQL handles millions of queries per second with proper indexing, read replicas, and connection pooling. Horizontal sharding is complex but possible (see BEP-123). Choosing NoSQL to "avoid scaling problems" without benchmarking the actual bottleneck is premature optimization.
...
- **BEP-121** — Indexing Deep Dive: how indexes work across SQL and NoSQL systems
- **BEP-122** — Replication Strategies: primary-replica, multi-primary, eventual consistency
- **BEP-123** — Partitioning and Sharding: horizontal scaling strategies for large datasets
- **BEP-140** — Data Modeling: translating domain concepts to database structures
