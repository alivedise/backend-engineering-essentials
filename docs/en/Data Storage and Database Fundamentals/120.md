...
- Vertical scaling has limits; horizontal sharding is possible but adds complexity (see BEE-123).
...
SQL databases do scale — PostgreSQL handles millions of queries per second with proper indexing, read replicas, and connection pooling. Horizontal sharding is complex but possible (see BEE-123). Choosing NoSQL to "avoid scaling problems" without benchmarking the actual bottleneck is premature optimization.
...
- **BEE-121** — Indexing Deep Dive: how indexes work across SQL and NoSQL systems
- **BEE-122** — Replication Strategies: primary-replica, multi-primary, eventual consistency
- **BEE-123** — Partitioning and Sharding: horizontal scaling strategies for large datasets
- **BEE-140** — Data Modeling: translating domain concepts to database structures
