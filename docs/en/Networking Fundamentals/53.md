...
This is how HTTP/2 is negotiated over TLS (see [BEE-5](5.md)2). Without ALPN, an extra round trip would be needed to switch protocols after the connection is established.
...
- [BEE-34](34.md) — Cryptography basics: symmetric encryption, hashing, digital signatures (the building blocks used in TLS)
- [BEE-50](50.md) — TCP/IP: TLS runs over TCP; understanding the TCP handshake clarifies total connection latency (TCP SYN + TLS = 2-3 RTT before data)
- [BEE-52](52.md) — HTTP/2 and HTTP/3: ALPN negotiates HTTP/2 over TLS; HTTP/3 uses QUIC which has TLS 1.3 built-in
- [BEE-54](54.md) — Load Balancers: TLS termination strategies, SNI-based routing, and certificate management at scale
