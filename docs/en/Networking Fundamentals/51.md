...
- [BEP-50 — TCP/IP and the Network Stack](./50.md): What happens after DNS resolves—the TCP connection lifecycle.
- [BEP-53 — TLS/SSL Handshake](./53.md): TLS adds another round-trip on top of TCP; DNS + TCP + TLS means three round-trips before the first HTTP byte.
- [BEP-51 — Load Balancers](./51.md): L7 load balancers as an alternative to DNS-based load balancing, with health checking.
- [BEP-261 — Timeouts](../Performance/261.md): DNS resolution is part of the total request latency; set DNS timeout budgets explicitly.
