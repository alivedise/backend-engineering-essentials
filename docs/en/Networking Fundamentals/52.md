...
- [BEP-50 — TCP/IP and the Network Stack](./50.md): TCP handshake, slow start, and why QUIC replaces TCP for HTTP/3.
- [BEP-53 — TLS/SSL Handshake](./53.md): ALPN extension, TLS 1.3 integration with QUIC, certificate negotiation.
- [BEP-54 — Load Balancers](./54.md): Layer 7 termination of HTTP/2 and HTTP/3, protocol translation to upstreams.
- [BEP-70 — REST API Design](../API%20Design/70.md): How protocol version affects API latency and client connection strategy.
- [BEP-205 — HTTP Caching](../Caching/205.md): Cache-Control semantics are defined in RFC 9110 and apply identically across all HTTP versions.
