...
Common uses:
- Signing API request parameters to prevent tampering (see [BEE-11](11.md) for JWT)
...
Digital signatures are the foundation of JWT (JWS), TLS certificates, code signing, and HTTPS certificate chains. See [BEE-11](11.md) for JWT-specific guidance.
...
For backend TLS configuration, see [BEE-53](53.md).
...
- [BEE-11](11.md) — JWT Signatures: applies HMAC-SHA256 and ECDSA to produce and verify JSON Web Tokens.
- [BEE-32](32.md) — Secrets Management: key storage, rotation, and access control for encryption keys and secrets.
- [BEE-53](53.md) — TLS Configuration: TLS version requirements, cipher suite selection, and certificate management.
