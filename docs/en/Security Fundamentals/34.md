...
Common uses:
- Signing API request parameters to prevent tampering (see BEE-11 for JWT)
...
Digital signatures are the foundation of JWT (JWS), TLS certificates, code signing, and HTTPS certificate chains. See BEE-11 for JWT-specific guidance.
...
For backend TLS configuration, see BEE-53.
...
Keys hardcoded in source code are committed to version control, visible in build artifacts, and shared across all deployments. Rotate them and use a secrets manager (see BEE-32).
...
- **BEE-11** — JWT Signatures: applies HMAC-SHA256 and ECDSA to produce and verify JSON Web Tokens.
- **BEE-32** — Secrets Management: key storage, rotation, and access control for encryption keys and secrets.
- **BEE-53** — TLS Configuration: TLS version requirements, cipher suite selection, and certificate management.
