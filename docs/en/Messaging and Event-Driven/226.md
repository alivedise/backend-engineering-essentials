...
# [BEP-226] Idempotent Message Processing
...
This BEP describes how to design message consumers so that receiving the same message twice produces the same observable outcome as receiving it once.

**Related BEPs:**
- [BEP-72](../API%20Design/72.md) — API idempotency (same principle applied at the HTTP layer)
- [BEP-164](164.md) — Exactly-once semantics
- [BEP-222](222.md) — Delivery guarantees
- [BEP-224](224.md) — Dead letter queues
...
This is not a bug. At-least-once delivery is a deliberate trade-off that prioritizes data durability over duplicate elimination. Exactly-once delivery is either a myth or a very expensive guarantee that shifts the deduplication complexity elsewhere (see BEP-164).
...
Consumers in an event-driven architecture are not protected by API-layer idempotency keys (see BEP-72). Each consumer must implement its own deduplication independently. An idempotent HTTP handler does not make a downstream Kafka consumer idempotent.
