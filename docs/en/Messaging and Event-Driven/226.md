...
**Related BEPs:**
- [BEE-72](../API%20Design/72.md) — API idempotency (same principle applied at the HTTP layer)
...
This is not a bug. At-least-once delivery is a deliberate trade-off that prioritizes data durability over duplicate elimination. Exactly-once delivery is either a myth or a very expensive guarantee that shifts the deduplication complexity elsewhere (see BEE-164).
...
Consumers in an event-driven architecture are not protected by API-layer idempotency keys (see BEE-72). Each consumer must implement its own deduplication independently. An idempotent HTTP handler does not make a downstream Kafka consumer idempotent.
