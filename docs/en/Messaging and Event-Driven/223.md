...
- **Used alongside CQRS (BEP-102)** — event sourcing is the natural write-side complement to CQRS's read/write separation.
...
- **BEP-102** — CQRS: separating read and write models; event sourcing is the natural write-side pairing
- **BEP-144** — Temporal data: querying state as of a point in time
- **BEP-164** — Idempotency: projectors must be idempotent to support safe replay
- **BEP-220** — Event streams: the infrastructure layer that carries events between producers and projectors
