...
- **BEE-102** — CQRS: separating read and write models; event sourcing is the natural write-side pairing
- **BEE-144** — Temporal data: querying state as of a point in time
- **BEE-164** — Idempotency: projectors must be idempotent to support safe replay
- **BEE-220** — Event streams: the infrastructure layer that carries events between producers and projectors
