...
# [BEP-221] Publish-Subscribe Pattern
...
See BEP-221 for a full treatment of delivery guarantees.
...
Configure a DLQ on every durable subscription. Route messages that exceed the retry limit to the DLQ for inspection, alerting, and manual replay. See BEP-221.
...
- **BEP-220** — Message queues vs event streams: when to use each model
- **BEP-221** — Delivery guarantees: at-most-once, at-least-once, exactly-once
- **BEP-221** — Dead letter queues and poison message handling
- **BEP-101** — DDD domain events: what to publish and why
