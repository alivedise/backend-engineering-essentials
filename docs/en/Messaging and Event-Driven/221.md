...
See BEE-221 for a full treatment of delivery guarantees.
...
Configure a DLQ on every durable subscription. Route messages that exceed the retry limit to the DLQ for inspection, alerting, and manual replay. See BEE-221.
...
- **BEE-220** — Message queues vs event streams: when to use each model
- **BEE-221** — Delivery guarantees: at-most-once, at-least-once, exactly-once
- **BEE-221** — Dead letter queues and poison message handling
- **BEE-101** — DDD domain events: what to publish and why
