...
# [BEP-225] Backpressure and Flow Control
...
Rate limiting (see BEP-266) constrains the rate of requests from a single client. Backpressure constrains the rate of data flow based on downstream capacity. They solve different problems:
...
- **BEP-220** — Messaging fundamentals: broker patterns that backpressure integrates with
- **BEP-260** — Circuit Breaker: service-level protection that complements backpressure
- **BEP-266** — Rate Limiting: request-rate control, a static form of backpressure
- **BEP-305** — Async Processing: patterns where backpressure is structurally required
