...
Structured log lines emitted when messages are routed to the DLQ enable log-based alerting (see BEE-321):
...
- **BEE-222** — Delivery guarantees: at-most-once, at-least-once, and exactly-once semantics
- **BEE-261** — Retry strategies: exponential backoff, jitter, and retry budgets
- **BEE-321** — Structured logging: log formats for DLQ alerts and incident correlation
