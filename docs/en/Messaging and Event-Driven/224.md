...
# [BEP-224] Dead Letter Queues and Poison Messages
...
The correct model: retry a bounded number of times with backoff (see BEP-261), then route to the DLQ and move on.
...
- **Typical sweet spot: 3–5 for most systems**, combined with exponential backoff (see BEP-261). This allows the system to survive brief outages without endlessly recycling unprocessable messages.
...
Structured log lines emitted when messages are routed to the DLQ enable log-based alerting (see BEP-321):
...
Setting `maxReceiveCount` to a very large number (or no limit) is the same as having no DLQ. A poison message with 1,000 retry attempts will block the queue for hours before reaching the DLQ. Pair a finite retry count with exponential backoff (BEP-261).
...
- **BEP-222** — Delivery guarantees: at-most-once, at-least-once, and exactly-once semantics
- **BEP-261** — Retry strategies: exponential backoff, jitter, and retry budgets
- **BEP-321** — Structured logging: log formats for DLQ alerts and incident correlation
