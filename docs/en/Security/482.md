...
- [BEE-10](../Authentication/10.md) -- Authentication vs Authorization: Zero Trust requires both on every request; understand the distinction before designing Zero Trust policy
- [BEE-14](../Authentication/14.md) -- RBAC vs ABAC Access Control Models: Zero Trust policies combine user attributes, device posture, and resource sensitivity — ABAC is the natural fit
- [BEE-34](../Security/34.md) -- Cryptographic Basics for Engineers: mTLS and certificate-based workload identity are the cryptographic foundation of Zero Trust networking
- [BEE-467](../Distributed Systems/467.md) -- Service-to-Service Authentication: covers the application-level patterns (JWT, API keys, mTLS) that implement Zero Trust between services
- [BEE-53](../Networking/53.md) -- TLS/SSL Handshake: mTLS extends the standard TLS handshake to require client certificate verification
