...
A common pattern in large deployments is **cluster per region**: each data center runs a full cache cluster, and replication between regions is handled at the application or database layer rather than at the cache layer (see [BEE-165](165.md) on eventual consistency). This avoids cross-region cache writes on every request.
...
- [BEE-123](123.md) -- Database Sharding: consistent hashing applies identically to partitioning databases across nodes.
- [BEE-200](200.md) -- Caching Fundamentals: cache-aside, write-through, write-behind patterns and the fundamentals this article builds on.
- [BEE-204](204.md) -- Cache Stampede and Thundering Herd: what happens when distributed cache nodes fail and keys miss simultaneously.
- [BEE-165](165.md) -- Eventual Consistency: cross-region cache coherence and the consistency tradeoffs in distributed systems.
