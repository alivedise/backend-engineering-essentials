...
- **Availability**: A single cache node is a single point of failure. When it crashes, every read goes to the database simultaneously -- a thundering herd (see BEP-204).
...
A common pattern in large deployments is **cluster per region**: each data center runs a full cache cluster, and replication between regions is handled at the application or database layer rather than at the cache layer (see BEP-165 on eventual consistency). This avoids cross-region cache writes on every request.
...
- **BEP-123** -- Database Sharding: consistent hashing applies identically to partitioning databases across nodes.
- **BEP-200** -- Caching Fundamentals: cache-aside, write-through, write-behind patterns and the fundamentals this article builds on.
- **BEP-204** -- Cache Stampede and Thundering Herd: what happens when distributed cache nodes fail and keys miss simultaneously.
- **BEP-165** -- Eventual Consistency: cross-region cache coherence and the consistency tradeoffs in distributed systems.
