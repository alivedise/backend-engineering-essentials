...
4. **Ignoring cache consistency with the source of truth.** If you update the database without invalidating or updating the cache, reads will return stale data. Design your write path to manage cache state explicitly (see write patterns above and BEE-201).
...
- **BEE-201** -- Cache Invalidation Strategies: how to keep the cache consistent with the source of truth.
- **BEE-202** -- Cache Eviction Policies: LRU, LFU, TTL, and how eviction affects hit ratio.
- **BEE-203** -- Distributed Caching: consistent hashing, sharding, and replication in multi-node caches.
- **BEE-205** -- HTTP Caching: Cache-Control headers, ETags, conditional requests, and CDN semantics.
