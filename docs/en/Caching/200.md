...
# [BEP-200] Caching Fundamentals and Cache Hierarchy
...
4. **Ignoring cache consistency with the source of truth.** If you update the database without invalidating or updating the cache, reads will return stale data. Design your write path to manage cache state explicitly (see write patterns above and BEP-201).
...
- **BEP-201** -- Cache Invalidation Strategies: how to keep the cache consistent with the source of truth.
- **BEP-202** -- Cache Eviction Policies: LRU, LFU, TTL, and how eviction affects hit ratio.
- **BEP-203** -- Distributed Caching: consistent hashing, sharding, and replication in multi-node caches.
- **BEP-205** -- HTTP Caching: Cache-Control headers, ETags, conditional requests, and CDN semantics.
