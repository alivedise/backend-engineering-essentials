...
TTL-based expiry is a form of cache invalidation (see **BEE-201**), not eviction. Eviction is driven by memory pressure; invalidation is driven by data staleness. The two operate independently.
:::
...
- **BEE-200** -- Caching Fundamentals: cache patterns (cache-aside, write-through, write-behind) and when to add a cache layer.
- **BEE-201** -- Cache Invalidation Strategies: TTL, event-driven invalidation, and write-through. Invalidation is driven by data staleness; eviction is driven by memory pressure. They interact but are separate concerns.
- **BEE-203** -- Distributed Caching: how eviction policies behave in clustered and sharded caches, and the implications of consistent hashing on per-node capacity.
