...
Once responses carry `Cache-Control`, ETag enables free revalidation when entries expire. The mechanism is identical to [BEE-9006](../caching/http-caching-and-conditional-requests.md), applied to a GraphQL URL.
...
- [BEE-9006](../caching/http-caching-and-conditional-requests.md) HTTP Caching and Conditional Requests — foundation; assumed prior reading for `Cache-Control` and `ETag` mechanics
- [BEE-4005](graphql-vs-rest-vs-grpc.md) GraphQL vs REST vs gRPC — high-level protocol comparison; this article deepens its single bullet on GraphQL caching
- [BEE-4008](graphql-federation.md) GraphQL Federation — adjacent operational concern in the GraphQL family
- [BEE-9001](../caching/caching-fundamentals-and-cache-hierarchy.md) Caching Fundamentals and Cache Hierarchy — TTL and invalidation concepts that underpin this article
- [BEE-13005](../performance-scalability/content-delivery-and-edge-computing.md) CDN Architecture — how edge nodes consume `Cache-Control` directives
- [BEE-4003](api-idempotency.md) Idempotency in APIs — referenced for context on REST's HTTP-level idempotency mechanisms, which GraphQL must reinvent at the schema layer
