...
- [BEE-1001](../auth/authentication-vs-authorization.md) -- Authentication vs Authorization: BOLA is an authorization failure, not an authentication failure — the user's identity is verified; their permission to access the specific object is not
- [BEE-1005](../auth/rbac-vs-abac.md) -- RBAC vs ABAC Access Control Models: RBAC alone cannot prevent BOLA; ABAC's attribute-based policies can encode ownership as an attribute and are a closer fit for object-level authorization
- [BEE-2008](owasp-api-security-top-10.md) -- OWASP API Security Top 10: BOLA is API1:2023 — the most critical API security risk
- [BEE-2015](server-side-request-forgery-ssrf.md) -- SSRF: both BOLA and SSRF are about missing authorization on which resources the system will access; SSRF on network resources, BOLA on data objects
- [BEE-18007](../multi-tenancy/database-row-level-security.md) -- Database Row-Level Security: PostgreSQL RLS can enforce object-level ownership at the database layer, providing defense-in-depth against application-layer BOLA bugs
