...
- **Overload prevention** — acts as a load shedding valve at the API layer (see BEE-264), preventing traffic spikes from propagating to databases and downstream services
...
From the client side (and for services calling other services), receiving a 429 is not an error in the traditional sense — it is a signal to back off. See BEE-261 for full retry-with-backoff guidance. Key points:
...
- **BEE-15** (API Keys) — API keys are the primary identifier for per-client rate limiting; key rotation and scoping affect limit enforcement
- **BEE-75** (Error Handling) — 429 is an application-level error and must be handled explicitly, not treated as an unexpected exception
- **BEE-261** (Retry Strategies and Exponential Backoff) — retry behavior after receiving 429; always includes jitter and respects `Retry-After`
- **BEE-264** (Graceful Degradation and Load Shedding) — rate limiting as one layer of load shedding; BEE-264 covers application-level shedding strategies
