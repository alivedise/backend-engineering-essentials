...
From the client side (and for services calling other services), receiving a 429 is not an error in the traditional sense — it is a signal to back off. See [BEE-261](261.md) for full retry-with-backoff guidance. Key points:
...
- [BEE-15](15.md) (API Keys) — API keys are the primary identifier for per-client rate limiting; key rotation and scoping affect limit enforcement
- [BEE-75](75.md) (Error Handling) — 429 is an application-level error and must be handled explicitly, not treated as an unexpected exception
- [BEE-261](261.md) (Retry Strategies and Exponential Backoff) — retry behavior after receiving 429; always includes jitter and respects `Retry-After`
- [BEE-264](264.md) (Graceful Degradation and Load Shedding) — rate limiting as one layer of load shedding; [BEE-264](264.md) covers application-level shedding strategies
