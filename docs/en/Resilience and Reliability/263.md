...
This is **resource exhaustion cascading failure**. It is subtle and common. A circuit breaker (BEP-260) trips when a dependency *fails*; it does not protect you from a dependency that is merely *slow*. Timeouts (BEP-261) bound how long a single call waits, but if 500 threads are all waiting up to 5 seconds simultaneously, you still exhaust the pool before any timeout fires.
...
1. **Timeout** (BEP-261) — Bound how long any single call can block.
2. **Bulkhead** — Bound how many concurrent calls can be in-flight to each dependency.
3. **Circuit Breaker** (BEP-260) — Stop calling dependencies that are failing or saturated.
...
- **BEP-241** (Worker Pools) — foundational concepts for sizing and managing thread pools
- **BEP-260** (Circuit Breaker Pattern) — stop calling failed dependencies; combine with bulkheads for full protection
- **BEP-261** (Timeouts and Deadlines) — bound call duration so partition slots are not held indefinitely
- **BEP-261** (Graceful Degradation) — define fallback behavior when a bulkhead partition is full
