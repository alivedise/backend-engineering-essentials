...
- **BEE-241** (Worker Pools) — foundational concepts for sizing and managing thread pools
- **BEE-260** (Circuit Breaker Pattern) — stop calling failed dependencies; combine with bulkheads for full protection
- **BEE-261** (Timeouts and Deadlines) — bound call duration so partition slots are not held indefinitely
- **BEE-261** (Graceful Degradation) — define fallback behavior when a bulkhead partition is full
