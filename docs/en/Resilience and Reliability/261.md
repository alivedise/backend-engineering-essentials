...
# [BEP-261] Retry Strategies and Exponential Backoff
...
2. The operation accepts an idempotency key that the server uses to deduplicate (see BEP-71 and BEP-161)
...
3. **Use circuit breakers** (BEP-260) — stop retrying entirely when a downstream service is confirmed down
4. **Fail fast with timeouts** (BEP-261) — bounded timeouts prevent retries from accumulating latency
...
Adding retry logic to a payment or order creation endpoint without an idempotency key results in duplicate transactions. Always confirm idempotency before retrying (BEP-71, BEP-161).
...
- **BEP-71** — Idempotency keys for safe retries
- **BEP-161** — Exactly-once delivery via deduplication
- **BEP-260** — Circuit breakers to stop retrying failed services
- **BEP-261** — Timeouts to bound retry latency
