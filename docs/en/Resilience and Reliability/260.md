...
The goal is **graceful degradation** (see BEE-264). The system should continue providing reduced functionality rather than failing completely.
...
Service B starts returning HTTP 503. Service A retries each call (per BEE-261). Each request to Service A now holds a thread for several seconds, waiting for Service B timeouts plus retry backoff. Under normal load, 50 concurrent checkout requests tie up 50 threads. Connection pool exhausted. Service A starts rejecting all requests — including calls that have nothing to do with inventory. The failure has cascaded.
...
| Python | `pybreaker` |
| Node.js | `opossum` |
| Service mesh | Envoy (Istio, Linkerd) — see BEE-105 |
...
- **BEE-261** (Retry Strategies and Exponential Backoff) — complement circuit breakers with retry for transient faults
- **BEE-262** (Timeouts and Deadlines) — always pair timeouts with circuit breakers; a long timeout delays circuit opening
- **BEE-263** (Bulkhead Pattern) — isolate thread pools to contain failure blast radius
- **BEE-264** (Graceful Degradation) — defines how to handle fallback behavior when the circuit is open
- **BEE-105** (Service Mesh) — service mesh provides circuit breaking as infrastructure-level policy
