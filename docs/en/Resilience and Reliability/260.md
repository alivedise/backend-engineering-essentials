...
# [BEP-260] Circuit Breaker Pattern
...
The goal is **graceful degradation** (see BEP-264). The system should continue providing reduced functionality rather than failing completely.
...
Service B starts returning HTTP 503. Service A retries each call (per BEP-261). Each request to Service A now holds a thread for several seconds, waiting for Service B timeouts plus retry backoff. Under normal load, 50 concurrent checkout requests tie up 50 threads. Connection pool exhausted. Service A starts rejecting all requests — including calls that have nothing to do with inventory. The failure has cascaded.
...
| Python | `pybreaker` |
| Node.js | `opossum` |
| Service mesh | Envoy (Istio, Linkerd) — see BEP-105 |

Service meshes (BEP-105) can implement circuit breaking as a sidecar, removing it from application code entirely. This is the preferred approach in Kubernetes environments for consistent cross-service policy.

## Related BEPs

- **BEP-261** (Retry Strategies and Exponential Backoff) — complement circuit breakers with retry for transient faults
- **BEP-262** (Timeouts and Deadlines) — always pair timeouts with circuit breakers; a long timeout delays circuit opening
- **BEP-263** (Bulkhead Pattern) — isolate thread pools to contain failure blast radius
- **BEP-264** (Graceful Degradation) — defines how to handle fallback behavior when the circuit is open
- **BEP-105** (Service Mesh) — service mesh provides circuit breaking as infrastructure-level policy
