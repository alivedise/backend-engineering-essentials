...
6. Service A retries (making the overload worse — see [BEE-261](./261.md)).
7. Service B's circuit breaker (see [BEE-260](./260.md)) should open, but it does not because Service B never returns an error — it just hangs.
...
- [BEE-50](../Networking/50.md) — TCP keepalive and socket-level timeouts
- [BEE-260](./260.md) — Circuit breaker: automatic fallback when a dependency is timing out consistently
- [BEE-261](./261.md) — Retry after timeout: idempotency, backoff, and retry budgets
- [BEE-302](../Performance/302.md) — Connection pooling: acquisition timeout and pool sizing
