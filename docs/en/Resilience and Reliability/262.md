...
# [BEP-262] Timeouts and Deadlines
...
6. Service A retries (making the overload worse — see [BEP-261](./261.md)).
7. Service B's circuit breaker (see [BEP-260](./260.md)) should open, but it does not because Service B never returns an error — it just hangs.
...
- [BEP-50](../Networking/50.md) — TCP keepalive and socket-level timeouts
- [BEP-260](./260.md) — Circuit breaker: automatic fallback when a dependency is timing out consistently
- [BEP-261](./261.md) — Retry after timeout: idempotency, backoff, and retry budgets
- [BEP-302](../Performance/302.md) — Connection pooling: acquisition timeout and pool sizing
