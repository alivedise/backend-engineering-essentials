...
# [BEP-240] Threads vs Processes vs Coroutines
...
Goroutines are cheap, but not free. Spawning a goroutine for every incoming request with no backpressure mechanism will exhaust memory under a spike. Always cap concurrency with a semaphore, worker pool, or rate limiter. See BEP-244 for worker pool patterns.
...
- **BEP-241** — Race Conditions: what goes wrong when threads share state without synchronization
- **BEP-242** — Locks and Mutual Exclusion: mutex patterns, deadlock, and lock ordering
- **BEP-243** — Async I/O: event loops, non-blocking syscalls, and the reactor pattern
- **BEP-244** — Worker Pools: bounding concurrency, backpressure, and graceful shutdown
