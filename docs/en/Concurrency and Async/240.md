...
Goroutines are cheap, but not free. Spawning a goroutine for every incoming request with no backpressure mechanism will exhaust memory under a spike. Always cap concurrency with a semaphore, worker pool, or rate limiter. See BEE-244 for worker pool patterns.
...
- **BEE-241** — Race Conditions: what goes wrong when threads share state without synchronization
- **BEE-242** — Locks and Mutual Exclusion: mutex patterns, deadlock, and lock ordering
- **BEE-243** — Async I/O: event loops, non-blocking syscalls, and the reactor pattern
- **BEE-244** — Worker Pools: bounding concurrency, backpressure, and graceful shutdown
