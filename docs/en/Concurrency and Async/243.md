...
# [BEP-243] Async I/O and Event Loops
...
This M:N scheduling (see BEP-240) means Go avoids both the callback/promise complexity of Node.js and the memory cost of OS-thread-per-connection. The tradeoff is that CPU-bound goroutines can still starve I/O-bound ones on a single P (logical processor), though Go's preemptive scheduler (since Go 1.14) mitigates this.
...
3. CPU-intensive work must leave the event loop: offload to a worker thread pool, a separate process, or a purpose-built CPU worker (see BEP-241).
...
│       │   └── Yes → offload to worker pool (see BEP-241)
│       └── Many concurrent DB queries?
│           └── Yes → use connection pooling (see BEP-301)
└── No, mostly CPU-bound?
...
- **BEP-240** — Threads vs Processes vs Coroutines: the concurrency primitives underlying async runtimes
- **BEP-241** — Worker Pools: offloading CPU-bound work from the event loop
- **BEP-301** — Connection Pooling: preventing connection exhaustion in async services
