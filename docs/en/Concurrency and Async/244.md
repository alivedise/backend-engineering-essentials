...
# [BEP-244] Producer-Consumer and Worker Pool Patterns
...
│   │       or use async I/O instead (see BEP-243)
│   │
...
└── No discrete tasks (streaming, continuous computation)?
    └── Consider async I/O (BEP-243) or reactive streams (BEP-305)
```

## Related BEPs

- **BEP-225** — Backpressure: system-level strategies for propagating load signals upstream
- **BEP-240** — Threads vs Processes vs Coroutines: the primitives that worker pools are built on
- **BEP-243** — Async I/O and Event Loops: the alternative to thread pools for I/O-bound work
- **BEP-305** — Async Processing and Job Queues: durable, distributed queues (Kafka, SQS) for cross-service task distribution
