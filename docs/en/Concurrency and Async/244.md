...
│   │       or use async I/O instead (see BEE-243)
│   │
...
- **BEE-225** — Backpressure: system-level strategies for propagating load signals upstream
- **BEE-240** — Threads vs Processes vs Coroutines: the primitives that worker pools are built on
- **BEE-243** — Async I/O and Event Loops: the alternative to thread pools for I/O-bound work
- **BEE-305** — Async Processing and Job Queues: durable, distributed queues (Kafka, SQS) for cross-service task distribution
