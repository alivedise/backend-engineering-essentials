...
**Choosing the right isolation level is an application design decision**, not a database default to accept blindly. See [BEP-161](./161.md) for a full treatment of isolation levels and the anomalies each one permits.
...
See [BEP-165](./165.md) for patterns and trade-offs in eventual consistency.
...
- **Two-Phase Commit (2PC):** A coordinator prepares all participants, then commits. Provides ACID-like guarantees but is slow, complex, and blocks on coordinator failure. See [BEP-162](./162.md).
- **Saga pattern:** Break the distributed operation into a sequence of local transactions with compensating actions for failure. See [BEP-163](./163.md).
...
Accepting the default isolation level without understanding it is a latent bug. The PostgreSQL default (Read Committed) allows non-repeatable reads and lost updates under concurrent load. Evaluate your workload against the anomalies each level permits. See [BEP-161](./161.md).
...
- [BEP-161: Isolation Levels and Their Anomalies](./161.md) -- deep dive into isolation levels and which anomalies each allows
- [BEP-162: Distributed Transactions and Two-Phase Commit](./162.md) -- coordinating transactions across services
- [BEP-163: Saga Pattern](./163.md) -- compensating transactions for long-running distributed workflows
- [BEP-165: Eventual Consistency Patterns](./165.md) -- when to trade ACID guarantees for availability
