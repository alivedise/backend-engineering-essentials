...
The gap between a write being committed and that write being visible to all replicas is called the **consistency window**. Monitor replication lag (see BEE-122). Alert if the window exceeds your SLA. Expose replication lag in your observability dashboards.
...
- **BEE-122** -- Replication Lag: measuring and alerting on the consistency window
- **BEE-160** -- ACID Transactions: when to use strong consistency instead
- **BEE-162** -- Distributed Transactions: coordinating writes across services
- **BEE-203** -- Distributed Caching: cache invalidation and staleness
