...
This M:N scheduling (see [BEE-24](24.md)0) means Go avoids both the callback/promise complexity of Node.js and the memory cost of OS-thread-per-connection. The tradeoff is that CPU-bound goroutines can still starve I/O-bound ones on a single P (logical processor), though Go's preemptive scheduler (since Go 1.14) mitigates this.
...
3. CPU-intensive work must leave the event loop: offload to a worker thread pool, a separate process, or a purpose-built CPU worker (see [BEE-24](24.md)1).
...
│       │   └── Yes → offload to worker pool (see [BEE-24](24.md)1)
│       └── Many concurrent DB queries?
│           └── Yes → use connection pooling (see [BEE-2001](../security-fundamentals/owasp-top-10-for-backend.md)1)
└── No, mostly CPU-bound?
...
- [BEE-11001](threads-vs-processes-vs-coroutines.md) — Threads vs Processes vs Coroutines: the concurrency primitives underlying async runtimes
- [BEE-11002](race-conditions-and-data-races.md) — Worker Pools: offloading CPU-bound work from the event loop
- [BEE-13002](../performance-scalability/horizontal-vs-vertical-scaling.md) — Connection Pooling: preventing connection exhaustion in async services
