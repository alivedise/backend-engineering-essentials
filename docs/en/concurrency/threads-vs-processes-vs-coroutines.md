...
Goroutines are cheap, but not free. Spawning a goroutine for every incoming request with no backpressure mechanism will exhaust memory under a spike. Always cap concurrency with a semaphore, worker pool, or rate limiter. See [BEE-11005](producer-consumer-and-worker-pool-patterns.md) for worker pool patterns.
...
- [BEE-11002](race-conditions-and-data-races.md) — Race Conditions: what goes wrong when threads share state without synchronization
- [BEE-11003](locks-mutexes-and-semaphores.md) — Locks and Mutual Exclusion: mutex patterns, deadlock, and lock ordering
- [BEE-11004](async-i-o-and-event-loops.md) — Async I/O: event loops, non-blocking syscalls, and the reactor pattern
- [BEE-11005](producer-consumer-and-worker-pool-patterns.md) — Worker Pools: bounding concurrency, backpressure, and graceful shutdown
