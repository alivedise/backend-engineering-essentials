...
- [BEE-200](../Caching/200.md) -- Caching Fundamentals: the in-memory suggestion cache is the critical path; cache miss handling and eviction policy directly affect autocomplete latency
- [BEE-204](../Caching/204.md) -- Cache Stampede and Thundering Herd: high-traffic autocomplete endpoints are vulnerable to stampedes when a popular prefix expires simultaneously
- [BEE-380](380.md) -- Full-Text Search Fundamentals: the inverted index that powers the result set behind the autocomplete entry point
- [BEE-381](381.md) -- Search Relevance Tuning: the same scoring signals used for result ranking inform the suggestion score at each prefix node
