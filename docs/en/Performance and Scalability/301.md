...
# [BEP-301] Horizontal vs Vertical Scaling
...
Read replicas address the common pattern where reads outnumber writes by 10:1 or more. Each replica is a full copy; queries are distributed across them. Write operations still go to the primary — see [BEP-121](/en/Data%20Management/121) for replication details.

Sharding partitions data across multiple independent database nodes. Each shard owns a subset of rows (by user ID range, hash, or geography). Writes scale because they are distributed. Reads require routing to the correct shard. See [BEP-123](/en/Data%20Management/123) for sharding mechanics.
...
3. **Correct scale-out metric** — CPU is common but can be wrong. A request-queue-depth or response-latency-based trigger is often more accurate. See [BEP-300](/en/Performance%20and%20Scalability/300) for capacity estimation and load testing.
...
- [BEP-51 — Load Balancers](/en/Infrastructure/51): routing traffic across horizontal instances
- [BEP-121 — Replication](/en/Data%20Management/121): horizontal read scaling at the data layer
- [BEP-123 — Sharding](/en/Data%20Management/123): horizontal write scaling at the data layer
- [BEP-300 — Back-of-Envelope Estimation](/en/Performance%20and%20Scalability/300): knowing when to scale
