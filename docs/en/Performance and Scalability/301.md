...
Read replicas address the common pattern where reads outnumber writes by 10:1 or more. Each replica is a full copy; queries are distributed across them. Write operations still go to the primary — see [BEE-121](121.md) for replication details.

Sharding partitions data across multiple independent database nodes. Each shard owns a subset of rows (by user ID range, hash, or geography). Writes scale because they are distributed. Reads require routing to the correct shard. See [BEE-123](123.md) for sharding mechanics.
...
3. **Correct scale-out metric** — CPU is common but can be wrong. A request-queue-depth or response-latency-based trigger is often more accurate. See [BEE-300](300.md) for capacity estimation and load testing.
