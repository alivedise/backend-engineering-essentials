...
- [BEE-509](509.md) -- RAG Pipeline Architecture: the two-stage retrieval pipeline described here is the production-grade retrieval component of a RAG system
- [BEE-516](516.md) -- Embedding Models and Vector Representations: bi-encoder embedding models power the dense retrieval stage; model selection and quantization decisions apply directly
- [BEE-383](../Search/383.md) -- Vector Search and Semantic Search: ANN index structures (HNSW, IVF) and approximate search are the underlying mechanism for the dense retrieval stage
- [BEE-512](512.md) -- LLM Context Window Management: the reranker's top-k output must fit within the allocated RAG context budget
