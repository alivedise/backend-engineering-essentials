...
- [BEE-503](503.md) -- LLM API Integration Patterns: the retry, timeout, and client configuration patterns for text APIs apply equally to multi-modal calls; vision and audio endpoints share the same HTTP client
- [BEE-512](512.md) -- LLM Context Window Management: images consume significant portions of the context window; the same token budget discipline applies to multi-modal inputs
- [BEE-513](513.md) -- AI Cost Optimization and Model Routing: image token costs dwarf text token costs at scale; cost routing logic must account for vision token pricing when selecting models
- [BEE-518](518.md) -- LLM Streaming Patterns: multi-modal responses stream the same way as text responses; the first streamed token appears after the vision encoder finishes, introducing higher TTFT for image-heavy requests
