...
- [BEE-525](525.md) -- Chain-of-Thought and Extended Thinking Patterns: self-consistency builds on chain-of-thought prompting; extended thinking is an alternative path to improved reasoning
- [BEE-527](527.md) -- LLM Batch Processing Patterns: self-consistency across large document sets is a batch workload; batch APIs reduce per-call cost for N samples
- [BEE-541](541.md) -- LLM Provider Rate Limiting: N-sample fan-out multiplies token consumption and requires quota headroom
- [BEE-513](513.md) -- AI Cost Optimization and Model Routing: route N samples to a cheaper model when the task allows, reserving the expensive model for the aggregation step
