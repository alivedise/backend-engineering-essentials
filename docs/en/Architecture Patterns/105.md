...
- A **simpler alternative exists**: a well-configured API gateway, a shared library, or a reverse proxy (see BEE-55) may solve the problem at a fraction of the cost.
...
mTLS proves that Service A is talking to Service B. It does not prove that the request is authorized. You still need application-level authorization (RBAC, JWT validation, business rules). BEE-53 covers mTLS; do not conflate transport security with application security.
...
- [BEE-53 — Mutual TLS (mTLS)](/en/Security/53): The security foundation the sidecar enforces
- [BEE-55 — Reverse Proxy Patterns](/en/Architecture%20Patterns/55): Simpler alternative for smaller deployments
- [BEE-100 — Microservices Architecture](/en/Architecture%20Patterns/100): The context in which service meshes become necessary
- [BEE-260 — Circuit Breaker Pattern](/en/Architecture%20Patterns/260): Reliability pattern the mesh automates
- [BEE-321 — Distributed Tracing](/en/Observability/321): Observability that sidecars enable automatically
