...
- A **simpler alternative exists**: a well-configured API gateway, a shared library, or a reverse proxy (see [BEE-5](5.md)5) may solve the problem at a fraction of the cost.
...
mTLS proves that Service A is talking to Service B. It does not prove that the request is authorized. You still need application-level authorization (RBAC, JWT validation, business rules). [BEE-53](53.md) covers mTLS; do not conflate transport security with application security.
