...
# [BEP-105] Sidecar and Service Mesh Concepts
...
- A **simpler alternative exists**: a well-configured API gateway, a shared library, or a reverse proxy (see BEP-55) may solve the problem at a fraction of the cost.
...
mTLS proves that Service A is talking to Service B. It does not prove that the request is authorized. You still need application-level authorization (RBAC, JWT validation, business rules). BEP-53 covers mTLS; do not conflate transport security with application security.
...
VirtualServices, DestinationRules, and AuthorizationPolicies interact. A misconfiguration can silently affect traffic. Before rolling out advanced mesh features, ensure you have distributed tracing (BEP-321) and metrics dashboards in place so misconfigurations surface quickly.

## Related BEPs

- [BEP-53 — Mutual TLS (mTLS)](/en/Security/53): The security foundation the sidecar enforces
- [BEP-55 — Reverse Proxy Patterns](/en/Architecture%20Patterns/55): Simpler alternative for smaller deployments
- [BEP-100 — Microservices Architecture](/en/Architecture%20Patterns/100): The context in which service meshes become necessary
- [BEP-260 — Circuit Breaker Pattern](/en/Architecture%20Patterns/260): Reliability pattern the mesh automates
- [BEP-321 — Distributed Tracing](/en/Observability/321): Observability that sidecars enable automatically
