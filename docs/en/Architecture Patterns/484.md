...
- [BEE-305](../Performance and Scalability/305.md) -- Asynchronous Processing and Work Queues: the SQS-triggered Lambda pattern is a serverless implementation of the worker queue model
- [BEE-163](../Transactions and Data Integrity/163.md) -- Saga Pattern: Step Functions state machines implement the saga pattern for long-running distributed transactions
- [BEE-164](../Transactions and Data Integrity/164.md) -- Idempotency and Exactly-Once Semantics: Lambda retries on failure make idempotent handlers mandatory
- [BEE-260](../Resilience and Reliability/260.md) -- Circuit Breaker Pattern: downstream dependencies called from Lambda still require circuit breakers; the serverless boundary does not eliminate cascading failure risk
- [BEE-475](../Observability/475.md) -- OpenTelemetry Instrumentation: distributed tracing across Lambda invocation chains requires explicit trace context propagation
