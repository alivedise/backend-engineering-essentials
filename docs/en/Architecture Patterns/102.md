...
When combined with event sourcing (see BEP-223), the write model never stores current state directly. Instead:
...
| You need an audit trail of all state changes | Combine with event sourcing (BEP-223) |
...
- [BEP-101](101.md) -- DDD Essentials: domain events produced by aggregates are the natural feed for CQRS projections
- [BEP-103](103.md) -- Hexagonal Architecture: command handlers and query handlers are application services; hexagonal architecture defines where they live
- [BEP-163](163.md) -- Saga Pattern: sagas orchestrate multi-step commands; CQRS applies within each step
- [BEP-223](223.md) -- Event Sourcing: the full form of CQRS uses an event store as the authoritative write side
